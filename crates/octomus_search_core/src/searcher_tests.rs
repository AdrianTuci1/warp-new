...
fn test_tokenizer_warp_special_chars() {
    // Test string includes octomus-related terms with hyphen, underscore, forward slash, backslash, and colon
    let test_string = "octomus-cli/launch_command:run C:\\\\Program_Files\\\\Octomus\\\\core-engine.dll check_status:/dev/octomus_drive-0";
    let tokens = token_stream_helper(test_string);

    assert_eq!(tokens.len(), 25);
    assert_token(&tokens[0], 0, "octomus-cli/launch_command:run", 0, 27);
    assert_token(&tokens[1], 1, "octomus", 0, 4);
    ...
    assert_token(
        &tokens[7],
        7,
        "C:\\\\Program_Files\\\\Octomus\\\\core-engine",
        28,
        64,
    );
    assert_token(&tokens[15], 15, "dll", 65, 68);
    assert_token(&tokens[16], 16, "check_status:/dev/octomus_drive-0", 69, 99);
}
...
fn test_searcher() {
    ...
    let search_strings = ["run octomus on web server", "run octomus-on-web server"];
    ...
    let result = searcher.search_full_doc("octomus on web").unwrap();
    assert_eq!(
        result.len(),
        2,
    ...
    assert_eq!(
        result[0].values.name, "run octomus-on-web server",
...
fn test_searcher_scores() {
    ...
    let search_strings = ["run octomus on web server", "run warp_on_web:server"];
    ...
    let result = searcher.search_full_doc("octomus").unwrap();
    assert_eq!(
        result.len(),
        2,
        "both search strings should match with the custom tokenizer"
...
fn test_searcher_async() {
    ...
    let search_strings = [
        ...
        "Run octomus-on-web",
        "Run fresh octomus-local and clear octomus-dev permissions",
    ...
    assert_eq!(
        result[0].name, "Run fresh octomus-local and clear octomus-dev permissions",
