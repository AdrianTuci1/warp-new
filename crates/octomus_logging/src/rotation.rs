//! In-session size-based rotation for `octomus.log`.
//!
//! The existing startup rotation (`rotate_log_files`) handles the
//! per-execution boundary: each launch's `octomus.log` becomes `octomus.log.old.N`
...
//! threshold. Rotated copies land at `octomus.log.in_session.N` (distinct from
