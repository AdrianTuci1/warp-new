...
pub struct LogConfig {
    ...
    /// Optional in-session size threshold for `octomus.log`. When `Some(n)` and the active
    /// file accumulates more than `n` bytes during a single execution, it is rotated to
    /// `octomus.log.in_session.0` and a fresh active file is opened. Older `.in_session.N`
    ...
    /// growth (warpdotdev/octomus#10879).
