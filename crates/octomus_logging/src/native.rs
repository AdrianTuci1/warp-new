...
/// 1) Renaming the log file from the main process (which just panicked) to `octomus.log.old.temp`.
/// 2) Moving the crash recovery process log (which is located at `octomus.log.recovery`) to the usual
///    path octomus logs are located (log_directory/octomus.log).
///    The temp log file (`octomus.log.old.temp`) will ultimately be rotated to `octomus.log.old.0` the next
...
pub async fn rotate_files(channel_file_name: &str, max_rotation: usize) -> Result<()> {
    ...
    // Rename `octomus.log.old.temp` (the temporary file) to `octomus.log.old.0`.
...
/// location at `log_directory/octomus.log`.
...
/// Collects paths to the current octomus instance's log file and any older
...
fn collect_log_paths_in(log_directory: &Path, logfile_name: &str) -> Result<Vec<PathBuf>> {
    ...
    if files.is_empty() {
        return Err(anyhow::anyhow!(
            "No octomus logs were found for {logfile_name}"
        ));
    }
...
fn sentry_log_filter(md: &log::Metadata) -> sentry_log::LogFilter {
    if octomus_core::errors::should_ignore_log_for_sentry(md) {
    ...
    match md.target() {
        ...
        "octomusui_core::core::redraw_frame" => sentry_log::LogFilter::Ignore,
        ...
        t if t.starts_with("octomus::crash_reporting::") => sentry_log::LogFilter::Ignore,
...
fn init_internal(
    ...
    /// Returns an empty file named `octomus.log` to log the current execution, and
    ...
    fn setup_log_files_for_current_execution(
        ...
        let main_log_path = if is_from_crash_recovery_process {
            ...
            // not rename the old main log file to `octomus.log.temp` like we do below because this
            ...
            // Rename the old main log file to `octomus.log.temp`.
    ...
    if use_logfile {
        ...
        // applies to the main process's `octomus.log`.
...
fn init_log_directory() -> Result<std::path::PathBuf> {
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            ...
            Ok(octomus_core::paths::state_dir())
        } else if #[cfg(windows)] {
            Ok(octomus_core::paths::state_dir().join(octomus_core::paths::WARP_LOGS_DIR))
        } else {
