...
/// Execution mode that Octomus is running under.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ExecutionMode {
    /// Octomus is running as a normal desktop app.
    App,
    /// Octomus is running as a CLI.
    Sdk,
    /// Octomus is running as the remote server daemon.
    RemoteServerDaemon,
}
...
impl ExecutionMode {
    ...
    pub fn client_id(&self) -> &'static str {
        match self {
            ExecutionMode::App => "octomus-app",
            ExecutionMode::Sdk => "octomus-cli",
            ExecutionMode::RemoteServerDaemon => "octomus-remote-server-daemon",
        }
    }
}

/// Model tracking the mode that Octomus is running in.
///
/// This gates functionality that's disabled when Octomus is running in SDK mode.
#[derive(Clone, Debug)]
...
impl AppExecutionMode {
    ...
    /// If true, Octomus is running in a sandbox like a Docker container or VM, rather than directly
    /// on a user machine.
...
/// Returns the current global client ID string ("octomus-app", "octomus-cli", or "octomus-remote-server-daemon").
