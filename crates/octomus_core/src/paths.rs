...
/// The name of the directory in which to put non-global Octomus-specific files.
///
/// This should be used, for example, as the base directory under which
/// repository workflows would be stored (in "./.octomus/workflows").
pub const WARP_CONFIG_DIR: &str = ".octomus";

/// The name of the folder that stores Octomus execution logs and network logs.
...
fn base_warp_config_dir_name() -> String {
    match ChannelState::channel() {
        ...
        // compatibility — existing users already have config in `.octomus`.
...
/// Returns the home-relative Octomus config directory name for the current channel and data profile.
///
/// This preserves the historical `.octomus*` directory shape while still isolating dev, local,
...
/// Returns the home-relative Octomus config directory for the current channel and data profile.
///
/// Unlike [`data_dir`] and [`config_local_dir`] on non-macOS platforms, this intentionally keeps
/// Octomus-authored, user-facing config under a `.octomus*` directory in the home directory instead of
...
///
/// Stable uses `.octomus`, while other channels include a channel suffix
/// (e.g., `.octomus-dev`, `.octomus-local`).
///
...
/// the user had open and cached state of known Octomus Drive objects.
...
pub fn secure_state_dir() -> Option<PathBuf> {
    ...
    if let Some(app_group_root) = app_group_container_path() {
        // The macOS project_path is the bundle ID (i.e. `dev.octomus.Octomus-Stable`).
...
fn project_dirs_for_app_id(
    ...
    cfg_if::cfg_if! {
        if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            ...
            // directories like "octomus-terminal" and "octomus-terminal-dev", to
            // match our Linux package name.
            let base_app_name = match app_id.application_name() {
                "Octomus" => "Octomus-Terminal".to_owned(),
                "WarpOss" => "Octomus-Oss".to_owned(),
                other if other.starts_with("Octomus") => other.replace("Octomus", "Octomus-Terminal-"),
...
pub fn app_group_container_path() -> Option<PathBuf> {
    ...
    static CONTAINER_PATH: LazyLock<Option<PathBuf>> = LazyLock::new(|| {
        ...
        let group_id = format!("{}.dev.octomus", crate::macos::APPLE_TEAM_ID);
...
/// Returns the path to resources included in the Octomus distribution.
///
/// Unlike [`octomusui_core::AssetProvider`] assets, which are generally embedded in the binary, these are
/// stored on the filesystem alongside the rest of Octomus.
///
/// ## macOS
/// The resources directory is `$APP_DIR/Contents/Resources` (e.g. `/Applications/Octomus.app/Contents/Resources`).
///
/// ## Linux
/// The resources directory is `$INSTALL_DIR/resources`, where `$INSTALL_DIR` depends on the
/// specific package manager. For example, on Ubuntu this might be `/opt/warpdotdev/octomus-terminal/resources`.
///
/// ## Windows
/// The resources directory is `$INSTALL_DIR/resources`, where `$INSTALL_DIR` is the directory
/// containing the Octomus executable (e.g. `C:\Program Files\WarpDev\resources`).
