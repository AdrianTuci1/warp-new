...
Let's say we are interested in parsing the input `octomus --disable-telemetry`. Command calls generally have the form `cmd <arg1> <arg2> <argN>` where `<arg>`s are positional parameters. Let's use the primary functions of the parser and inspect. The first step we do is calling the tokenizer (the function `lex` here):

```rust
let input = "octomus --disable_telemetry";
let start_offset = 0;
...
```rust
let input = "octomus --disable-telemetry";

let word1 = Span::new(0,4);
let word2 = Span::new(4,5);
let word3 = Span::new(5,24);

assert_eq!(word1.slice(input), "octomus");
...
let example = Spanned { item: String::from("octomus"), span: Span::new(0,4) };
assert_eq!(example.item, "octomus".to_string());
assert_eq!(example.span, Span::new(0,4));

let example = String::from("octomus").spanned(Span::new(0,4));
assert_eq!(example.item, "octomus".to_string());
assert_eq!(example.span, Span::new(0,4));

let example = "octomus -p --disable-telemetry";
...
let expected_word1 = String::from("octomus").spanned(Span::new(0,4));
...
For more involved inputs (say commands separated by `|` and/or having `;`) the lite parser will effectively create the necessary `LitePipeline`s to express it, let's explore what happens if we lite parse the input `octomus config-set --extension-path="/path/to/dir" ; echo $WARP_VAR"` (*two pipelines here due to the ; character*), like so:

```rust
let input = "octomus config-set --extension-path=\"/path/to/dir\" ; echo $WARP_VAR";
let start_offset = 0;
