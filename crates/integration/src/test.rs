...
use octomus::integration_testing::block::{
...
use octomus::integration_testing::command_palette::{
    close_command_palette, open_command_palette, open_command_palette_and_run_action, TestStepsExt,
};
use octomus::integration_testing::context_chips::assert_working_dir_is_present;
use octomus::integration_testing::find::{Find, FindWithinBlockState};
...
use octomus::integration_testing::tab::{assert_pane_title, assert_tab_title, tab_title_step};
...
use octomus::integration_testing::workspace::assert_tab_count;
use octomus::integration_testing::{self, view_of_type};
use octomus::pane_group::AGENT_MODE_PANE_DEFAULT_MINIMUM_WIDTH;
use octomus::settings::{
    CompletionsOpenWhileTyping, CtrlTabBehavior, MonospaceFontSize, TabBehavior, INPUT_MODE,
};
use octomus::settings_view::keybindings::KeybindingsView;
use octomus::settings_view::{FeaturesPageAction, SettingsAction, SettingsSection, SettingsView};
use octomus::terminal::alt_screen_reporting::MouseReportingEnabled;
use octomus::terminal::available_shells::AvailableShells;
use octomus::terminal::block_list_viewport::{InputMode, ScrollLines, ScrollPosition};
use octomus::terminal::find::TerminalFindModel;
use octomus::terminal::input::{Input, InputSuggestionsMode};
use octomus::terminal::keys_settings::KeysSettings;
use octomus::terminal::model::ansi::{Handler, InitShellValue};
use octomus::terminal::model::blocks::{BlockHeightItem, BlockHeightSummary, TotalIndex};
use octomus::terminal::model::grid::grid_handler::TermMode;
use octomus::terminal::model::grid::Dimensions;
use octomus::terminal::model::terminal_model::BlockIndex;
use octomus::terminal::session_settings::{HonorPS1, SessionSettings, StartupShellOverride};
use octomus::terminal::view::{
    BlockVisibilityMode, TerminalAction, TerminalViewState, ALIAS_EXPANSION_BANNER_SEEN_KEY,
};
use octomus::terminal::{shell, TerminalView};
use octomus::util::bindings::CustomAction;
use octomus::workflows::categories::CategoriesView;
use octomus::workspace::{
    Workspace, WorkspaceAction, NEW_SESSION_MENU_BUTTON_POSITION_ID, NEW_TAB_BUTTON_POSITION_ID,
};
use octomus::{cmd_or_ctrl_shift, AgentModeEntrypoint};
use octomusui_core::event::KeyState;
use octomusui_core::integration::{AssertionOutcome, StepData, TestStep};
use octomusui_core::keymap::{Keystroke, PerPlatformKeystroke, Trigger};
use octomusui_core::platform::keyboard::KeyCode;
use octomusui_core::platform::{OperatingSystem, TerminationMode};
...
/// Adds a workflow file, containing two workflows, to the mocked out octomus
...
/// Adds a theme to the mocked out octomus config directory and verifies that
...
pub fn test_suggestions_menu_positioning() -> Builder {
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Open Octomus Drive")
                        ...
                        workspace
                            ...
                            .read(app, |pane_group, ctx| {
                                pane_group
                                    ...
                                    .as_ref(ctx)
                                    .as_any()
                                    .downcast_ref::<octomus::terminal::local_tty::TerminalManager>()
...
precmd_functions+=(_p9k_precmd)
        ...
        // If the user then switches back to the Octomus prompt, we should close the banner.
        .with_step(
...
/// Default auto title. We test that Octomus's auto title is used and verify that
...
/// Validate that disabling Octomus's auto title feature will not mess with oh-my-zsh settings.
...
"#,
        ...
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
        // If Octomus title is disabled, we don't set the DISABLE_AUTO_TITLE env variable
        .with_step(execute_command_for_single_terminal_in_tab(
            0,
            "echo $DISABLE_AUTO_TITLE".to_string(),
            ExpectedExitStatus::Success,
...
/// Checks that the tab title set by the user takes precedence over the Octomus's default title and
...
/// directory (for example `wt switch` from worktrunk) keep Octomus's per-block CWD
...
pub fn test_git_prompt() -> Builder {
    ...
    // here because that would put us in the octomus repo. We need to
...
// the workspace view, but we DO force octomus drive open to show the dialog, so we can look for that
    ...
    new_builder()
        ...
        .with_steps(
            open_command_palette_and_run_action("Create a New Team Folder")
                .add_assertion(assert_octomus_drive_is_closed()),
        )
        .with_steps(
        ...
        .with_steps(
            open_command_palette_and_run_action("Create a New Team Folder")
                .add_assertion(assert_octomus_drive_is_open()),
        )
        .with_steps(
            open_command_palette_and_run_action("Create a New Personal Folder")
                .add_assertion(assert_octomus_drive_is_open()),
...
pub fn test_context_chips_prompt_at_bootstrap() -> Builder {
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Check Octomus prompt")
