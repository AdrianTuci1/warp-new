...
impl Client {
    ...
    /// Helper method to determine if the request should include octomus-specific headers. The only case
    /// where we should include custom headers is if the request is same-origin and is targetted to our server.
    /// For example, app.localhost --> app.localhost.
    ...
    fn include_warp_http_headers<U: IntoUrl + Clone>(url: U) -> bool {
        url.into_url().is_ok_and(|url| {
            url.host_str().is_some_and(|dest_host| {
                ...
                // If the request is going to our server, the destination host should be "app.localhost" or
                // "staging.localhost". The window hostname should also return the same.
