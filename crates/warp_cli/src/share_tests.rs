...
fn test_parse_user_default() {
    let result = parse_share_request("ben@localhost:8080").unwrap();
    match result.subject {
        ShareSubject::User { email } => assert_eq!(email, "ben@localhost:8080"),
...
fn test_parse_user_view() {
    let result = parse_share_request("ben@localhost:8080:view").unwrap();
    match result.subject {
        ShareSubject::User { email } => assert_eq!(email, "ben@localhost:8080"),
        _ => panic!("Expected User subject"),
    }
...
fn test_parse_user_edit() {
    let result = parse_share_request("ben@localhost:8080:edit").unwrap();
    match result.subject {
        ShareSubject::User { email } => assert_eq!(email, "ben@localhost:8080"),
        _ => panic!("Expected User subject"),
    }
