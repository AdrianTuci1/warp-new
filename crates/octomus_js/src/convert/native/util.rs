    ...
    T: FromWarpJs<'js>,
{
    ...
    if value.is_array() {
        Vec::<T>::from_octomus_js(ctx, value, js_function_registry)
    } else {
        Ok(vec![T::from_octomus_js(ctx, value, js_function_registry)?])
    }
}
    ...
    T: FromWarpJs<'js>,
{
    ...
    T::from_octomus_js(ctx, value, js_function_registry)
}
