...
@implementation NSPasteboard (Octomus)
...
- (BOOL)acceptsFirstMouse:(NSEvent *)event {
    ...
    // and we explicity fire the event here so that Octomus can handle it.
...
- (void)mouseDown:(NSEvent *)event {
    if (self.readyForWarp) {
        ...
        if (self->titlebarDragEnabled && !eventHandled && [self mouseInTitleBar:event]) {
            // If Octomus doesn't do anything with the event, indicated by returning `false`, and
