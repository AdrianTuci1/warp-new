...
- (id)init {
    ...
    // octomusui to still be run via `cargo run` since the app is not bundled in this case. Note this
...
- (void)keyWindowChanged:(NSNotification *)note {
    ...
    //  2. When a new window is created, octomus will activate it; if we recursively call back into
    //     octomus then we will cause the app to be mutably borrowed while already borrowed.
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center
    ...
    // Handle what happens when the user clicks the notification. Octomus doesn't support any actions
...
WarpApplication *get_warp_app() {
    ...
    dispatch_once(&once, ^{
      ...
      // Hack to work around the fact that octomus is frequently tested as a
...
// The pointer will be provided back to Octomus in the callbacks (see menus.h).
// The result is autoreleased.
