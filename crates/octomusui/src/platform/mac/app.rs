...
use octomusui_core::assets::AssetProvider;
use octomusui_core::integration::TestDriver;
use octomusui_core::keymap::{Keystroke, Trigger};
use octomusui_core::modals::{AlertDialog, ModalId};
use octomusui_core::platform::app::{AppCallbackDispatcher, ApproveTerminateResult};
use octomusui_core::platform::menu::{Menu, MenuBar};
use octomusui_core::platform::{self, FilePickerCallback, SaveFilePickerCallback};
use octomusui_core::{AppContext, Event};
...
extern "C" {
    // Implemented in ObjC to get the octomus NSApplication subclass.
...
impl App {
    ...
    pub(in crate::platform) fn run(
        ...
        autoreleasepool(|_| {
            ...
            // SAFETY: `get_warp_app()` returns the octomus NSApplication subclass instance.
...
pub unsafe extern "C-unwind" fn warp_app_will_finish_launching(this: &mut Object) {
    ...
    if app.callbacks.has_internet_reachability_changed_callback() {
        // `setReachabilityListener` is a custom octomus app-delegate selector.
    ...
    if let Some(dock_menu_builder) = app.dock_menu_builder.take() {
        ...
        // `setDockMenu:` is a custom octomus app-delegate selector.
