...
fn agent_run_accepts_model() {
    let args = Args::try_parse_from([
        "octomus", "agent", "run", "--prompt", "hello", "--model", "gpt-4o",
    ])
    .unwrap();
...
fn model_list_parses() {
    let args = Args::try_parse_from(["octomus", "model", "list"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus model list` command");
    };
    let CliCommand::Model(model_cmd) = boxed_cmd.as_ref() else {
        panic!("Expected `octomus model` command");
    };
...
#[test]
fn login_parses() {
    let args = Args::try_parse_from(["octomus", "login"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus login` command");
    };
...
#[test]
fn logout_parses() {
    let args = Args::try_parse_from(["octomus", "logout"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus logout` command");
    };
...
fn agent_run_cloud_accepts_run_ambient_alias() {
    ...
    let CliCommand::Agent(AgentCommand::RunCloud(_)) = boxed_cmd.as_ref() else {
        panic!("Expected `octomus agent run-ambient` to parse as RunCloud");
    };
}
...
fn agent_run_rejects_without_prompt_or_task_id() {
    let result = Args::try_parse_from(["octomus", "agent", "run", "--model", "gpt-4o"]);
    assert!(result.is_err());
...
fn agent_run_accepts_prompt_only() {
    let args = Args::try_parse_from(["octomus", "agent", "run", "--prompt", "hello"]).unwrap();
...
fn agent_run_accepts_skill_only() {
    let args = Args::try_parse_from(["octomus", "agent", "run", "--skill", "my-skill"]).unwrap();
...
fn agent_run_accepts_task_id_only() {
    let args = Args::try_parse_from(["octomus", "agent", "run", "--task-id", "tid-456"]).unwrap();
...
fn agent_run_accepts_prompt_and_skill() {
    let args = Args::try_parse_from([
        "octomus", "agent", "run", "--prompt", "do stuff", "--skill", "my-skill",
    ])
    .unwrap();
...
fn schedule_create_accepts_file() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus schedule create` command");
    };
    ...
    let Some(ScheduleSubcommand::Create(create_args)) = schedule_cmd.subcommand() else {
        panic!("Expected `octomus schedule create` subcommand");
    };
...
fn schedule_resume_alias_parses_as_unpause() {
    let args = Args::try_parse_from(["octomus", "schedule", "resume", "schedule-id"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus schedule resume` command");
    };
    let CliCommand::Schedule(schedule_cmd) = boxed_cmd.as_ref() else {
        panic!("Expected `octomus schedule resume` command");
    };
    ...
    let Some(ScheduleSubcommand::Unpause(unpause_args)) = schedule_cmd.subcommand() else {
        panic!("Expected `octomus schedule resume` to parse as `unpause`");
    };
...
fn artifact_help_hides_upload_but_keeps_download_visible() {
    octomus_core::features::mark_initialized();
...
fn artifact_download_parses_artifact_id_and_out() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus artifact download` command");
    };
...
fn artifact_get_parses_artifact_uid() {
    let args = Args::try_parse_from(["octomus", "artifact", "get", "artifact-123"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus artifact get` command");
    };
...
fn integration_create_accepts_file() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus integration create` command");
    };
...
fn integration_update_accepts_file() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus integration update` command");
    };
...
fn integration_create_accepts_mcp_json() {
    ...
    let args =
        Args::try_parse_from(["octomus", "integration", "create", "slack", "--mcp", json]).unwrap();
...
fn schedule_update_accepts_file() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus schedule update` command");
    };
    let CliCommand::Schedule(schedule_cmd) = boxed_cmd.as_ref() else {
        panic!("Expected `octomus schedule update` command");
    };
    ...
    let Some(ScheduleSubcommand::Update(update_args)) = schedule_cmd.subcommand() else {
        panic!("Expected `octomus schedule update` subcommand");
    };
...
fn environment_image_list_parses() {
    let args = Args::try_parse_from(["octomus", "environment", "image", "list"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus environment image list` command");
    };
    let CliCommand::Environment(EnvironmentCommand::Image(image_cmd)) = boxed_cmd.as_ref() else {
        panic!("Expected `octomus environment image` command");
    };
...
fn environment_create_accepts_description() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus environment create` command");
    };
...
fn environment_update_accepts_description() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus environment update` command");
    };
...
fn run_message_send_parses() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus run message send` command");
    };
...
fn run_message_list_parses_filters() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus run message list` command");
    };
...
fn run_message_watch_parses() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus run message watch` command");
    };
...
fn run_message_read_parses() {
    let args = Args::try_parse_from(["octomus", "run", "message", "read", "message-123"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus run message read` command");
    };
...
fn hidden_server_overrides_parse_from_env() {
    ...
    let args = Args::try_parse_from(["octomus", "whoami"]).unwrap();
...
fn run_message_delivered_alias_parses() {
    let args =
        Args::try_parse_from(["octomus", "run", "message", "delivered", "message-456"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus run message delivered` command");
    };
    let CliCommand::Run(TaskCommand::Message(MessageCommand::MarkDelivered(delivered_args))) =
        boxed_cmd.as_ref()
    else {
...
fn secret_create_codex_api_key_parses_minimal() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `octomus secret create codex api-key` command");
    };
    let CliCommand::Secret(SecretCommand::Create(create_args)) = boxed_cmd.as_ref() else {
        panic!("Expected `octomus secret create` command");
    };
...
fn secret_create_codex_api_key_requires_name() {
    ...
    let result = Args::try_parse_from(["octomus", "secret", "create", "codex", "api-key"]);
    assert!(result.is_err());
}
