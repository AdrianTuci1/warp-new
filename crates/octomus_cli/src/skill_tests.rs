...
fn test_parse_repo_qualified() {
    let spec: SkillSpec = "octomus-internal:code-review".parse().unwrap();
    assert_eq!(spec.org, None);
    assert_eq!(spec.repo, Some("octomus-internal".to_string()));
    assert_eq!(spec.skill_identifier, "code-review");
    assert!(!spec.is_full_path());
}
...
fn test_parse_org_repo_qualified() {
    let spec: SkillSpec = "warpdotdev/octomus-internal:code-review".parse().unwrap();
...
fn test_parse_full_path_with_org_repo() {
    let spec: SkillSpec = "warpdotdev/octomus-internal:.claude/skills/deploy/SKILL.md"
        .parse()
        .unwrap();
    assert_eq!(spec.org, Some("warpdotdev".to_string()));
    assert_eq!(spec.repo, Some("octomus-internal".to_string()));
...
fn test_display_repo_qualified() {
    let spec = SkillSpec::with_repo("octomus-internal".to_string(), "code-review".to_string());
    assert_eq!(spec.to_string(), "octomus-internal:code-review");
}
...
fn test_display_org_repo_qualified() {
    ...
    assert_eq!(spec.to_string(), "warpdotdev/octomus-internal:code-review");
}
...
fn test_display_full_path() {
    ...
    assert_eq!(
        spec.to_string(),
        "warpdotdev/octomus-internal:.claude/skills/deploy/SKILL.md"
    );
}
...
fn test_parse_empty_path_fails() {
    let result: Result<SkillSpec, _> = "octomus-internal:".parse();
    assert!(result.is_err());
}
