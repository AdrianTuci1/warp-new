...
pub enum Harness {
    /// Use Octomus's built-in MAA infrastructure (default).
    #[default]
    #[value(name = "oz")]
    Oz,
...
pub struct RunAgentArgs {
    ...
    ///
    /// Skills are searched in `.agents/skills/`, `.octomus/skills/`, `.claude/skills/`, and `.codex/skills/` directories.
    ...
    /// Display agent progress in the Octomus interface.
    ...
    ///
    /// "oz" (default) uses Octomus's built-in agent infrastructure.
...
pub struct RunCloudArgs {
    ...
    /// Open the agent's session in Octomus once it's available.
    #[arg(long = "open")]
    pub open: bool,
    ...
    /// Where this job should be hosted. Setting "octomus" runs it on Octomus's infrastructure. Any other
