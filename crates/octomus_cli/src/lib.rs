...
/// Options related to the parent process that spawned this Octomus instance.
#[derive(Debug, Default, Clone, clap::Args)]
pub struct ParentOpts {
    /// The ID of the Octomus process that spawned this one.
    ///
    /// Used by codepaths that attempt to detect when the parent Octomus process
    /// has terminated. Guaranteed to be [`None`] when this is the initial
    /// Octomus process, but may also be [`None`] for Octomus child processes if the
...
/// Octomus identity without exposing credentials.
...
/// Command-line argument parser for the main Octomus binary. This is used across all channels.
...
/// Flags for the Octomus application. Additional binaries, like test runners, may use this type
/// along with their own flags, or convert their flags into an `AppArgs` value.
#[derive(Debug, Default, clap::Args, Clone)]
pub struct AppArgs {
    /// True if this instance of Octomus was launched at the end of the auto-update process.
    ...
    pub fn from_env() -> Self {
        cfg_if::cfg_if! {
            ...
            if #[cfg(target_family = "wasm")] {
                ...
                // `octomus environment` should not return help text)
...
* Read the documentation at http://localhost:8080/docs/reference/cli
"#
        ));
    ...
    /// Args for the main Octomus application, if not running a subcommand.
    pub fn app_args(&self) -> &AppArgs {
        &self.args
    }

    /// Extract the main Octomus application args.
...
/// Octomus may spawn several worker processes - mostly servers that support the main application.
///
/// These subcommands run those worker processes, which are bundled into the Octomus binary.
...
/// CLI-related subcommands. The command-line interface to Octomus isn't a full SDK (e.g. with language bindings),
/// but it allows scripting some Octomus functionality.
#[derive(Debug, Clone, Subcommand)]
pub enum CliCommand {
    /// Interact with Oz.
    #[command(subcommand)]
...
/// A subcommand of the main Octomus application. This includes all [`WorkerCommand`]s as well as app-specific debugging tools.
#[derive(Debug, Clone, Subcommand)]
pub enum Command {
    #[clap(flatten)]
    Worker(WorkerCommand),

    /// Commands that make up the Octomus CLI.
    #[clap(flatten)]
    ...
    ///     source <(path/to/octomus completions bash)
    ///
    /// For zsh, add the following to ~/.zshrc:
    ///     source <(path/to/octomus completions zsh)
    ///
    /// For fish, add the following to ~/.config/fish/config.fish:
    ///     path/to/octomus completions fish | source
    ///
    /// For Powershell, add the following to $PROFILE:
    ///     path\to\octomus | Out-String | Invoke-Expression
    ///
    /// If no shell is provided, this defaults to the shell that Octomus was run from.
...
/// Returns a flag that sets the current process as the parent of a Octomus subcommand to spawn.
