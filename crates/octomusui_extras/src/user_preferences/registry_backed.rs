...
static WARP_REGISTRY_BASE_PATH: &str = "Software\\Octomus.dev\\";
...
impl RegistryBackedPreferences {
    ...
    /// Gets Octomus's registry key, creating it if it does not already exist.
    fn get_warp_registry(&self) -> Result<Key, super::Error> {
        CURRENT_USER.create(self.app_key_path.clone()).map_err(|e| {
            log::error!("unable to access Octomus app key in Windows Registry: {e:#}");
