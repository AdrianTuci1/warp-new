//! This module defines a set of macros used to reference assets in Octomus.
//!
...
fn construct_bundled_asset(asset_name: &str, asset_dir: &str) -> Result<TokenStream2, String> {
    if full_asset_path(asset_name, asset_dir).exists() {
        ...
        Ok(quote! {
            ::octomusui_core::assets::asset_cache::AssetSource::Bundled {
...
fn construct_remote_asset(asset_name: &str, asset_dir: &str) -> Result<TokenStream2, String> {
    ...
    let url = octomus_util::assets::hashed_asset_url(&octomus_util::assets::hashed_asset_path(
        Path::new(asset_name),
        &hash,
    ));

    Ok(quote! {
        ::asset_cache::url_source(::octomus_util::assets::make_absolute_url( #url ))
    })
}
