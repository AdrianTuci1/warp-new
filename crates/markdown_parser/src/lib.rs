...
/// This purposefully shadows the `Action` trait from `octomusui`.
///
/// Since `octomusui` depends on this crate, we can't depend on the `octomusui_core::Action` trait directly.
/// Instead, we create a new trait with a blanket implementation that implicitly results
/// in any `octomusui_core::Action` implementing this `Action`.
...
impl FormattedTable {
    /// Parse from the internal tab-separated format used in `octomus-markdown-table` code blocks.
    ...
    /// Serialize to the internal tab-separated format used in `octomus-markdown-table` code blocks.
