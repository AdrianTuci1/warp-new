...
fn test_find_applicable_rules_no_matching_rules() {
    ...
    rules.upsert_rule(Path::new("/x/y/OCTOMUS.md"), "content1".to_string());
...
fn test_find_applicable_rules_single_matching_rule() {
    ...
    rules.upsert_rule(Path::new("/a/OCTOMUS.md"), "content1".to_string());
    ...
    assert_eq!(result[0].path, PathBuf::from("/a/OCTOMUS.md"));
}
...
fn test_find_applicable_rules_includes_all_ancestor_rules() {
    ...
    rules.upsert_rule(Path::new("/a/OCTOMUS.md"), "root_warp".to_string());
    rules.upsert_rule(Path::new("/a/b/OCTOMUS.md"), "nested_warp".to_string());
    rules.upsert_rule(Path::new("/a/b/c/OCTOMUS.md"), "deep_warp".to_string());
    ...
    // All should be OCTOMUS.md files (same priority), order is not specified by depth
    // Just verify all expected rules are present
    let paths: Vec<PathBuf> = result.iter().map(|r| r.path.clone()).collect();
    assert!(paths.contains(&PathBuf::from("/a/OCTOMUS.md")));
    assert!(paths.contains(&PathBuf::from("/a/b/OCTOMUS.md")));
    assert!(paths.contains(&PathBuf::from("/a/b/c/OCTOMUS.md")));
}
...
fn test_find_applicable_rules_multiple_patterns() {
    ...
    rules.upsert_rule(Path::new("/a/OCTOMUS.md"), "warp_content".to_string());
    ...
    assert_eq!(result[1].path, PathBuf::from("/a/OCTOMUS.md"));
...
fn test_find_applicable_rules_exact_path_match() {
    ...
    rules.upsert_rule(Path::new("/a/b/OCTOMUS.md"), "exact_match".to_string());

    let path = PathBuf::from("/a/b/file.rs");

    let result = rules.find_active_or_applicable_rules(&path).active_rules;
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].path, PathBuf::from("/a/b/OCTOMUS.md"));
...
fn test_find_applicable_rules_ignores_deeper_paths() {
    ...
    rules.upsert_rule(Path::new("/a/OCTOMUS.md"), "applicable".to_string());
    rules.upsert_rule(Path::new("/a/b/c/d/e/OCTOMUS.md"), "too_deep".to_string()); // Path doesn't contain /a/b
...
fn test_find_applicable_rules_handles_root_path() {
    ...
    rules.upsert_rule(Path::new("/OCTOMUS.md"), "root_rule".to_string());

    let path = PathBuf::from("/a/b/file.rs");

    let result = rules.find_active_or_applicable_rules(&path).active_rules;
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].path, PathBuf::from("/OCTOMUS.md"));
...
fn test_find_applicable_rules_complex_scenario() {
    ...
    rules.upsert_rule(Path::new("/a/OCTOMUS.md"), "a_warp".to_string());
    rules.upsert_rule(Path::new("/a/AGENTS.md"), "a_agents".to_string());
    rules.upsert_rule(Path::new("/a/b/OCTOMUS.md"), "ab_warp".to_string());
    rules.upsert_rule(Path::new("/a/b/AGENTS.md"), "ab_agents".to_string());
    rules.upsert_rule(Path::new("/x/OCTOMUS.md"), "irrelevant".to_string()); // Should be ignored
    ...
    // Expect only OCTOMUS.md files to be included as they have higher priority.
    assert_eq!(result[0].path, PathBuf::from("/a/OCTOMUS.md"));
    assert_eq!(result[0].content, "a_warp");
    assert_eq!(result[1].path, PathBuf::from("/a/b/OCTOMUS.md"));
...
fn test_find_applicable_rules_handles_unknown_file_patterns() {
    ...
    rules.upsert_rule(Path::new("/a/OCTOMUS.md"), "known_pattern".to_string());
...
fn test_find_applicable_rules_with_relative_paths() {
    ...
    rules.upsert_rule(Path::new("src/OCTOMUS.md"), "src_warp".to_string());
    rules.upsert_rule(
        Path::new("src/components/OCTOMUS.md"),
    ...
    // Both are OCTOMUS.md files (same priority), order within same priority is not guaranteed
    // Just verify both rules are present
    let paths: Vec<PathBuf> = result.iter().map(|r| r.path.clone()).collect();
    assert!(paths.contains(&PathBuf::from("src/OCTOMUS.md")));
    assert!(paths.contains(&PathBuf::from("src/components/OCTOMUS.md")));
}
...
fn test_merge_independent_deltas() {
    let mut delta = RulesDelta {
        discovered_rules: vec![make_rule_path("/a/OCTOMUS.md")],
        deleted_rules: vec![],
    };
    delta.merge(RulesDelta {
        discovered_rules: vec![],
        deleted_rules: vec![PathBuf::from("/b/OCTOMUS.md")],
    });
...
fn test_merge_add_then_delete_yields_delete() {
    ...
    delta.merge(RulesDelta {
        discovered_rules: vec![],
        deleted_rules: vec![PathBuf::from("/a/OCTOMUS.md")],
    });

    assert!(delta.discovered_rules.is_empty());
    assert_eq!(delta.deleted_rules, vec![PathBuf::from("/a/OCTOMUS.md")]);
}
...
// the model. Bypasses the watcher infrastructure (which requires the octomusui
...
fn test_in_dir_warp_shadows_agents_with_global() {
    let mut model = ProjectContextModel::default();
    insert_global_rule(&mut model, Path::new("/home/u/.agents/AGENTS.md"), "global");
    // Both OCTOMUS.md and AGENTS.md in the same project directory: OCTOMUS.md should
    ...
    // Expect: [global, project OCTOMUS.md]. project AGENTS.md is shadowed.
    assert_eq!(result.active_rules.len(), 2);
    assert_eq!(result.active_rules[0].content, "global");
    assert_eq!(result.active_rules[1].content, "project_warp");
}
...
fn test_multiple_global_rules_all_contribute() {
    ...
    insert_global_rule(
        &mut model,
        Path::new("/home/u/.octomus/OCTOMUS.md"),
        "warp_global",
    );
