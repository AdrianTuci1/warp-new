//! Node.js and npm runtime management for Octomus.
//!
...
/// 3. Extract it to the Octomus data directory
///
/// # Returns
...
pub async fn install_npm(client: &http_client::Client) -> Result<PathBuf> {
    ...
    let node_containing_dir = octomus_core::paths::data_dir().join("node");
...
/// 1. First, our custom Node.js installation in the Octomus data directory
...
pub async fn detect_system_node(path_env_var: impl AsRef<OsStr>) -> Result<()> {
    ...
    // via Octomus's inherited env rather than the captured interactive PATH.
    #[cfg(windows)]
