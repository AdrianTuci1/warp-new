...
use octomusui_core::event::{DispatchedEvent, ModifiersState};
...
/// and the parent view can overlay UI controls on top of them using a [`octomusui_core::elements::Stack`].
///
...
impl<V: EditorView> Element for RichTextElement<V> {
    ...
    fn paint(&mut self, origin: Vector2F, ctx: &mut PaintContext, app: &AppContext) {
        ...
        ctx.scene
            .start_layer(octomusui_core::ClipBounds::BoundedBy(clip_bounds));
...
impl<V: EditorView> NewScrollableElement for RichTextElement<V> {
    ...
    fn scroll(&mut self, delta: octomusui_core::units::Pixels, axis: Axis, ctx: &mut EventContext) {
...
impl<V: EditorView> ScrollableElement for RichTextElement<V> {
    ...
    fn scroll(&mut self, delta: octomusui_core::units::Pixels, ctx: &mut EventContext) {
