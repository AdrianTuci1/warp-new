...
fn no_proxy_suffix_with_dot() {
    ...
    clear_proxy_env();
    env::set_var("HTTPS_PROXY", "http://proxy:3128");
    env::set_var("NO_PROXY", ".localhost:8080");

    assert!(resolved_proxy_tls("sessions.app.localhost:8080").is_none());

    assert!(resolved_proxy_tls("localhost:8080").is_some()); // Exact "localhost:8080" != ".localhost:8080"
    assert!(resolved_proxy_tls("other.com").is_some());
    clear_proxy_env();
}
...
fn no_proxy_suffix_without_dot() {
    ...
    // "sessions.app.localhost:8080" ends with ".localhost:8080" → matches
    assert!(resolved_proxy_tls("sessions.app.localhost:8080").is_none());
    // Exact match too
    assert!(resolved_proxy_tls("localhost:8080").is_none());
    assert!(resolved_proxy_tls("notlocalhost:8080").is_some());
    clear_proxy_env();
}
