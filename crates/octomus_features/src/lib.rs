...
pub enum FeatureFlag {
    ...
    /// If `true`, fetch updated Octomus channel versions from the Octomus server endpoint instead of
    /// from GCP directly.
    ...
    /// Whether the user is part of the Octomus Alpha Program (AI Trusted Testers).
    ...
    /// Enable multiselect in Notebooks and Octomus Text.
    RichTextMultiselect,
    ...
    /// Enables next action prediction within Octomus, powered by AI.
    AgentPredict,

    /// Enables receiving shared Octomus Drive objects.
    SharedWithMe,
    ...
    /// Playground for reducing Octomus UI clutter.
    MinimalistUI,
    ...
    /// Adds aliases for executing Octomus Drive workflows.
    WorkflowAliases,
    ...
    /// Enables support for Octomus Packs.
    WarpPacks,
    ...
    /// Enables Octomus Drive objects (like workflows) as context in AI context menu
    ...
    /// Enables the /create-environment slash command for setting up Octomus Environments
    ...
    /// Enables Octomus Managed Secrets functionality.
    WarpManagedSecrets,
    ...
    /// Enables auto-syncing ambient plans to Octomus Drive.
    SyncAmbientPlans,

    /// Enables platform skills support (--skill flag) for agent runs.
    ///
    /// Skills are loaded from `.agents/skills/`, `.octomus/skills/`, `.claude/skills/`, and `.codex/skills/`
    ...
    /// Enables the OpenWarp launch modal announcing Octomus going open-source.
    ...
    /// Ctrl-G intercepts the keystroke and opens Octomus's input editor instead of $EDITOR.
    CLIAgentRichInput,
    ...
    /// Enables the new settings to control visibility of Octomus Drive, Code Review Panel,
    ...
    /// CLIs (e.g. `claude`) to execute prompts instead of Octomus's agent harness.
    AgentHarness,

    /// Enables workspace- and block-snapshot handoff between cloud agent runs
    /// and the local Octomus client.
    /// When enabled:
    ...
    /// Enables the install/update chip for the OpenCode Octomus plugin.
    /// Requires HOANotifications to also be enabled.
    OpenCodeNotifications,

    /// Enables the install/update chip for the Codex Octomus notification plugin.
    /// Requires HOANotifications to also be enabled.
    CodexNotifications,

    /// Enables the Codex Octomus plugin marketplace integration.
    /// When disabled, Codex uses native OSC9 notifications.
    CodexPlugin,

    /// Enables the install/update chip for the Gemini CLI Octomus extension.
    /// Requires HOANotifications to also be enabled.
    GeminiNotifications,
    ...
    /// Replaces the in-block octomusification banner with a octomusify footer.
    OctomusifyFooter,
...
pub const DOGFOOD_FLAGS: &[FeatureFlag] = &[
    ...
    // These are enabled via 100% experiment on prod octomus-server,
...
/// Features enabled for feature preview build users (e.g.: Friends of Octomus).
...
impl FeatureFlag {
    ...
    pub fn flag_description(&self) -> Option<&'static str> {
        ...
        match self {
            ...
            CloudEnvironments => Some("Enables creating and managing Octomus Environments via the CLI."),
            CreateEnvironmentSlashCommand => Some("Enables the /create environment slash command for setting up Octomus Environments with custom configurations."),
            ...
            SettingsFile => Some("Enables configuring Octomus via a user-editable `settings.toml` file, with hot reload and error reporting for invalid values."),
