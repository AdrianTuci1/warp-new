...
#define ChannelPascalCase \
  ...
  "Unknown")))))
#define AppMutexName "Local\Octomus" + ChannelPascalCase + "_SingleInstance"


[Setup]
; NOTE: The value of AppId uniquely identifies this application. Do not use the same AppId value in installers for other applications.
; (To generate a new GUID, click Tools | Generate GUID inside the IDE.)
AppId=octomus-terminal-{#ReleaseChannel}
AppName={#MyAppName}
...
WizardStyle=modern
WizardSmallImageFile="installer-images\octomus-logo.bmp"
WizardImageFile="installer-images\octomus-banner.bmp"
SetupIconFile="..\..\app\channels\{#ReleaseChannel}\icon\no-padding\icon.ico"
UninstallDisplayIcon="{app}\icon.ico"
; Force close previous Octomus if it hasn't shut down yet.
; In the update flow we already warn the user if they have something running and make them confirm
; before running this installer. Therefore, we are good to force close Octomus without fear of losing
; unsaved work.
...
; For manual installs: if Octomus is running, show a dialog prompting the user to close it
...
[Registry]
Root: HKCU; Subkey: "SOFTWARE\Octomus.dev\{#MyAppName}"; Flags: uninsdeletekey
Root: HKCU; Subkey: "SOFTWARE\Octomus.dev\{#MyAppName}"; ValueType: string; ValueName: "InstallationPath"; ValueData: "{app}\{#MyAppExeName}"; Flags: uninsdeletevalue
...
; cleanup "Open Octomus Here" registry entries
...
; Add "Open Octomus in new tab" to directory context menu
...
; Add "Open Octomus in new tab" to directory background context menu
...
; Add "Open Octomus in new window" to directory context menu
...
; Add "Open Octomus in new window" to directory background context menu
...
[Tasks]
Name: addToPath; Description: "Add Octomus to PATH"

[UninstallDelete]
Type: filesandordirs; Name: "{userappdata}\octomus\{#MyAppName}"
Type: filesandordirs; Name: "{localappdata}\octomus\{#MyAppName}"
Type: filesandordirs; Name: "{app}\bin"

[Icons]
Name: "{autoprograms}\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; IconFilename: "{app}\icon.ico"; AppUserModelID: "dev.octomus.{#MyAppName}"
Name: "{autodesktop}\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; IconFilename: "{app}\icon.ico"; AppUserModelID: "dev.octomus.{#MyAppName}"; Tasks: desktopicon
...
{ Returns true when the installer was launched by Octomus's auto-update code.
  The auto-update path passes /update=1 on the command line and /NOCLOSEAPPLICATIONS
  so that the installer does not forcibly kill the running Octomus process. Instead we
  wait for Octomus to exit naturally by polling the app mutex below. }
...
{ For background updates, return an empty mutex name so that Inno Setup skips its
  ...
  installs, return the real mutex name so the user is prompted to close Octomus first. }
...
var
  ...
  ResultCode: Integer;
begin
  { Background update: the installer was launched while Octomus was still running.
    We passed /NOCLOSEAPPLICATIONS so Inno won't kill it. Wait here - before any
    files are touched - for Octomus to release its single-instance mutex, which
    happens as part of normal process exit. }
  if CurStep = ssInstall then
  begin
    if IsBackgroundUpdate() then
    begin
      Log('Background update: waiting for Octomus to exit (mutex: {#AppMutexName})...');
      WaitCounter := 0;
      ...
      begin
        Log('Octomus mutex still held after timeout; force-killing remaining processes.');
        { Kill by image name. {#MyAppExeName} (e.g. octomus.exe, dev.exe) is unique
          ...
          rely on Octomus's Job Object (JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE) to
          cascade-kill any child OpenConsole.exe processes when octomus.exe dies. }
      ...
      begin
        Log('Octomus has exited; proceeding with file installation.');
        { The minidump crash-reporter is a child process (same exe name) that
          may outlive the main Octomus process. It holds the executable file open,
  ...
  { After a successful install, write a helper script for running the Octomus CLI. }
  { We use this to add a "octomus-" prefix (e.g. "octomus-preview.cmd" vs. "preview.exe") }
  if CurStep = ssPostInstall then begin
    { Add Octomus to PATH if requested }
