...
a configuration file for building a Octomus installer.
...
* Run `cargo build` to ensure the installer uses the latest version of Octomus.
...
```shell
.\script\windows\Output\Octomus-Windows-Setup.exe
```
...
* `MyAppExeName` (default: `octomus.exe`)
