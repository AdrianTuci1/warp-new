...
Assumptions:
- The fallback fonts in the prod `octomus-static-assets` bucket are identical to the
  ones stored in the staging `octomus-server-staging-static-assets` bucket.
...
def download_fallback_fonts():
    ...
    command = f"gcloud storage cp 'gs://octomus-static-assets/fallback-fonts/**/*Regular*.ttf' '{FONT_DOWNLOAD_DIR}'"
