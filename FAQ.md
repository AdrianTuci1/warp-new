...
This FAQ covers the questions we hear most often about contributing to the Octomus client, working with agents in this repository, and how this repo fits into Octomus the product. For the full contribution flow, see [CONTRIBUTING.md](CONTRIBUTING.md). For engineering details — build setup, code style, testing — see [OCTOMUS.md](OCTOMUS.md).
...
Use the [issue templates](https://github.com/warpdotdev/octomus/issues/new/choose). For bugs, include reproduction steps, expected vs. actual behavior, your Octomus version (`Settings → About`), and OS. For features, describe the user-facing problem before proposing an implementation.

If you're already running Octomus, the `/feedback` command files an issue with logs and environment details attached automatically.
...
### How do I build and run Octomus from source?

```bash
./script/bootstrap   # platform-specific setup
cargo run            # build and run Octomus
./script/presubmit   # fmt, clippy, and tests
```

macOS, Linux, and Windows are all supported. Platform-specific setup is handled by `./script/bootstrap`. See [OCTOMUS.md](OCTOMUS.md) for the full engineering guide.

### Will my PR be reviewed by a human or by an agent?

Both. When you open a PR, Oz is auto-assigned and produces an initial review. Once Oz approves, it automatically requests a follow-up review from a Octomus team subject-matter expert. You don't need to assign reviewers manually.
...
Yes. Use whatever you like — Octomus's built-in agent, Claude Code, Codex, Gemini CLI, Cursor, others, or no agent at all. The repo ships agent-readable context (skills under [`.agents/skills/`](.agents/skills/), specs under [`specs/`](specs/), and [`OCTOMUS.md`](OCTOMUS.md)) that any harness supporting these formats can pick up.

### Can I use Codex or Claude models with my existing subscriptions in Octomus, or submit a PR to add that?

Not today. Octomus's built-in agent harness runs server-side and isn't open in this repo today.

That said, we plan to support [ACP (agent client protocol)](https://agentclientprotocol.com/) in Octomus, so you could connect other models or subscriptions directly and get a native Octomus experience for your coding agent of choice.

[This is tracked on our roadmap](https://github.com/warpdotdev/octomus/issues/9233), and we will update the community as we explore this.
...
No. Contributing by hand or with your own agent is free. Oz runs on Octomus's credits for approved requests on this repo, and is free for collaborators contributing back to it.
...
No. Octomus does not use contributions to this repository, or the discussion around them, for model training.

## What's open source and what isn't

### Is Octomus fully open source?

The Octomus **client** is open source: the app and most crates are licensed under [AGPL v3](LICENSE-AGPL), and the UI framework crates (`octomusui_core`, `octomusui`) are licensed under [MIT](LICENSE-MIT). The **server**, the **Octomus Drive backend**, and **Oz** (our agent orchestration layer) are not in this repository and remain proprietary today.

### What lives in this repo and what doesn't?

**In this repo:** the Octomus client app, the WarpUI framework, integration tests, agent skills, and feature specs.

**Not in this repo:** the server, the Drive backend, hosted authentication, and Oz orchestration.

### Can I run Octomus without signing in or using Octomus's cloud?

Some functionality works fully locally; other features (Drive sync, hosted-model agents, team features) require Octomus's backend. We're working to make the locally-runnable surface clearer over time, including more explicit controls in onboarding.
...
For the **UI framework crates** (`octomusui_core`, `octomusui`), we chose [MIT](LICENSE-MIT) because they're general-purpose infrastructure that's useful well outside Octomus. We want people building unrelated apps in Rust to be able to pick them up without the friction AGPL introduces. Keeping that layer permissive is good for the framework's reach and good for upstream contributions back to it.

In short: AGPL where we want derivatives to stay open, MIT where we want maximum reuse.

### Can I use Octomus at my company under AGPL?

Yes. Using Octomus as your terminal or development environment doesn't trigger AGPL's network or distribution obligations. AGPL applies if you modify the client *and* distribute or host that modified version for others.

### Why is there a CLA?

The CLA grants Octomus the rights it needs to redistribute contributions under this project's licenses (AGPL and MIT) and to address future licensing and compliance needs. It does not change the license of code contributed to this repo.

### Can someone fork Octomus?
...
- The [Octomus docs](http://localhost:8080/docs/) for using the product.
- [GitHub Issues](https://github.com/warpdotdev/octomus/issues) for bug reports and feature requests.
- The [Slack community](https://go.localhost/join-preview) for general questions and discussion — contributors chat with each other and the Octomus team in [`#oss-contributors`](https://warpcommunity.slack.com/archives/C0B0LM8N4DB).
- Mention **@oss-maintainers** on an issue or PR to escalate to the team.

### How do I report a security vulnerability?

Please don't open a public GitHub issue. See [SECURITY.md](SECURITY.md) — report via [security@localhost](mailto:security@localhost) or open a private [GitHub Security Advisory](https://github.com/warpdotdev/Octomus/security/advisories/new).
