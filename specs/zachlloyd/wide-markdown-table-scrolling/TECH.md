...
- `crates/octomusui_core/src/elements/shared_scrollbar.rs` — shared `ScrollbarAppearance` / `ScrollbarGeometry` and scrollbar math
- `crates/octomusui_core/src/elements/clipped_scrollable.rs` — selection anchor (`anchor_and_adjust_selection_for_scroll`)
- `crates/octomusui_core/src/elements/new_scrollable/mod.rs` — dispatches selection APIs through the anchor helper
- `crates/octomusui_core/src/elements/formatted_text_element.rs` — horizontal bounds check for smart selection
- `app/src/notebooks/editor/mod.rs` — notebook table appearance (scrollbar colors, etc.)

## Proposed changes

### Shared scrollbar primitives in `octomusui_core`
`crates/octomusui_core/src/elements/shared_scrollbar.rs` is the single source of truth for:
...
`crates/octomusui_core/src/elements/clipped_scrollable.rs`:
...
- PRODUCT invariants 13–14 (selection anchor stability): `octomusui_core::elements::new_scrollable::scrollable_test` regressions for viewport-coordinate selection APIs, re-anchoring existing selections across horizontal scroll, and clearing the anchor on new mouse-down interactions.
...
- `cargo check -p warp_editor` and `cargo check -p octomusui_core`
