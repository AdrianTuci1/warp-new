...
- **Compile gating**: `cargo check -p octomus` and `cargo check -p octomus --features queued_prompts_v2` both pass; `cargo fmt` and `cargo clippy --workspace --all-targets --all-features --tests -- -D warnings` per the OCTOMUS.md PR workflow.
