...
- Already used in `octomus_util::path` for MSYS2/WSL path conversion and in `ai::paths` for cross-platform path joining and normalization.
...
New struct in `octomus_util::path` (or a new `octomus_util::standardized_path` module), wrapping `TypedPathBuf`:
...
**Phase 1 (this work):**
- Add `StandardizedPath` to `octomus_util`.
