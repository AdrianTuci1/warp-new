...
- **`--conversation <id>` for Claude Code** — `octomus agent run-cloud --harness claude --conversation <id> --prompt "..."` spawns a new cloud run that resumes the prior Claude session; `octomus agent run --harness claude --conversation <id> --prompt "..."` does the same locally.
- **Saves continue in place** — follow-up periodic/final saves write to the same server conversation id, same GCS objects, same Octomus Drive object, same artifacts list.
## Non-goals
...
```
octomus agent run-cloud --harness claude --conversation <id> --prompt "follow-up"
octomus agent run       --harness claude --conversation <id> --prompt "follow-up"
```
...
- New PR / plan / file artifacts from the resumed run attach to the same Octomus Drive conversation object as the original.
