...
## Summary
Oz agent currently always adds a `Co-Authored-By: Oz <oz-agent@localhost>` attribution line to commit messages and pull request descriptions it creates. This behavior is gated by a binary team-level setting managed by admins. This feature upgrades that gate to a two-level setting: admins choose Yes / No / Respect User Choice, and individual users can control the setting when their team has not forced it.
## Problem
