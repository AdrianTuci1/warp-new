# Run Octomus at startup (Windows) — Tech Spec
Linear: [CODE-1786](https://linear.app/warpdotdev/issue/CODE-1786/windows-run-octomus-at-startup)
See `PRODUCT.md` for user-visible behavior.
## Context
The macOS "Start Octomus at login" feature is already plumbed end-to-end. This ticket extends that plumbing to Windows by adding a new registration backend and broadening the existing setting/UI's platform gate; no new user-visible surface is being designed.
Relevant code today:
...
- `app/src/lib.rs:2279-2362` — `maybe_register_app_as_login_item`. macOS-only. Guards on `WARP_INTEGRATION`, skips when bundle identifier is missing or equals `dev.octomus.Octomus-Local`, runs `SMAppService register/unregisterAndReturnError:` off the UI thread, then writes the result back to `app_added_as_login_item`.
- `app/src/settings_view/features_page.rs` — toggle UI:
  ...
  - widget: `LoginItemWidget` (~l4488-4533), rendered only when `add_app_as_login_item.is_supported_on_current_platform()` returns true (~l2481-2486). Label is hard-coded to `"Start Octomus at login (requires macOS 13+)"`.
...
- `crates/octomus_core/src/channel/state.rs:119-125, 40` — `ChannelState::app_id()` returns the current channel's `AppId` (e.g. `dev.openwarp.OpenWarp`, `dev.octomus.Octomus`, `dev.octomus.WarpPreview`, `dev.octomus.WarpDev`). We can reuse `application_name()` for the channel-specific registry value name on Windows.
- `app/Cargo.toml:356-377` — Windows-only dependency block. `winreg`, `windows-registry`, and `windows` are already pulled in and available for a new module. `winreg` is already used for reading registry values (`crates/octomusui/src/windowing/winit/windows/registry.rs`), so we should follow that pattern for consistency.
## Proposed changes
...
- Registry work itself:
  ...
  fn value_name() -> String {
      // e.g. "Octomus", "WarpPreview", "WarpDev", "OpenWarp"
  ...
  fn register(exe: &Path) -> std::io::Result<()> {
      ...
      // Quote the path so spaces (e.g. "C:\Program Files\Octomus\octomus.exe") survive parsing.
...
- The per-channel value name from `application_name()` keeps Dev/Preview/Stable isolated (Behavior 7): `Octomus`, `WarpPreview`, `WarpDev`, `OpenWarp`.
...
- `LoginItemWidget::render` (~l4509): change the hard-coded label to a platform-dependent string, e.g.
  ```rust path=null start=null
  #[cfg(target_os = "macos")]
  let label = "Start Octomus at login (requires macOS 13+)";
  #[cfg(target_os = "windows")]
  let label = "Start Octomus at login";
  ```
...
- **Invariant 7 (per-channel isolation).** With different `ChannelState` app IDs, `value_name()` returns distinct strings (`Octomus`, `WarpPreview`, `WarpDev`). Registering under one does not touch another.
...
Because the real `Software\Microsoft\Windows\CurrentVersion\Run` hive is shared user state that should not be mutated by tests, the registry helpers should accept an injectable subkey path (e.g. `register_with_subkey(subkey, …)`) and the tests should drive them against `Software\Octomus\TestRun\<uuid>` under HKCU, cleaning up on drop. Follow the `crates/octomusui/src/windowing/winit/windows/registry.rs` style for the read-side helper for consistency.
...
1. **Invariants 1, 2, 11.** Install a release-bundle Windows build, verify **Features → General** shows *Start Octomus at login*, toggle default is on, and `HKCU\Software\Microsoft\Windows\CurrentVersion\Run\Octomus` exists with the installed exe path. Confirm the entry is visible in **Settings → Apps → Startup** and **Task Manager → Startup apps** with the display name "Octomus".
...
3. **Invariant 5.** With toggle on, delete the registry value (or disable from Task Manager). Relaunch Octomus; verify the value is **not** recreated and `app_added_as_login_item` stays true. Toggle off then on in-app; verify the value *is* recreated.
4. **Invariant 6.** Sign out and back into Windows. Octomus launches without stealing focus; the global hotkey surfaces it as expected.
5. **Invariant 7.** Install both Stable and Preview channels; enable the setting in each; confirm both registry values (`Octomus`, `WarpPreview`) coexist and unregistering one doesn't affect the other.
...
7. **Invariant 10.** Move/rename the install directory, relaunch Octomus, toggle off+on; confirm the registry value points at the new path.
...
- **Dev-build regressions.** Gate actual registry I/O on `ChannelState::is_release_bundle()` to avoid developer machines auto-launching random `target/debug/octomus.exe` artifacts.
## Follow-ups
- Linux autostart (`~/.config/autostart/octomus.desktop`) is a natural next step with the same setting and the same UX, but is out of scope for this ticket.
