...
2. Octomus writes the default template to `~/.octomus/tab_configs/my_tab_config.toml` and opens it in the user's configured editor.
...
- Oz generates a valid `.toml` file and writes it to `~/.octomus/tab_configs/`.
...
- Octomus opens a new tab with the specified pane layout, running any configured commands.
...
- `worktree_name_autogenerated` (optional, bool, default false): when `true`, Octomus auto-generates the worktree branch name instead of prompting the user. The app detects worktree configs by scanning `commands` for `git worktree`.
...
8. The `tab-configs` bundled skill appears in `/skills` in Octomus.
