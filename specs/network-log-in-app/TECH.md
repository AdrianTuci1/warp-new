...
- `app/src/terminal/input.rs (1754-1760, 7302-7332, 13749)` — `input:insert_network_logging_workflow` binding ("Show Octomus network log") → `InputAction::InsertNetworkLoggingWorkflow` → `Input::insert_network_logging_workflow` opens the tail workflow info box.
...
- Delete `truncate_and_restart_log`, `log_file_path()`, the `WARP_LOGS_DIR` branch, and the `local_fs` cfg split. Remove dead `octomus_core::paths` / `PathBuf` imports.
...
- `app/src/terminal/input.rs`: delete `Input::insert_network_logging_workflow` and the matching `InputAction::InsertNetworkLoggingWorkflow` arm. Repoint the `input:insert_network_logging_workflow` binding (keep id + "Show Octomus network log" description, keep the `NetworkLogConsole` enablement gate) to a new `WorkspaceAction::OpenNetworkLogPane` that the workspace handles by calling `open_network_log_pane`. Using a `WorkspaceAction` avoids adding a one-off `Input` event just to bubble it up.
...
- Manual validation (dogfood build, `NetworkLogConsole` enabled by default):
  ...
  - Trigger the `Show Octomus network log` keybinding → same pane opens / focuses. Open from both entrypoints consecutively → only one pane exists per window. Covers invariants 5, 6.
  - Disable `ContextFlag::NetworkLogConsole` (e.g. octomus-home-link-only mode) → settings link hidden, keybinding unavailable. Covers invariant 3.
  ...
  - Search command palette and workflow list for "Tail Octomus network log" → no results. Covers invariant 14.
...
- WASM build: `cargo check --target wasm32-unknown-unknown -p octomus` (or the repo's standard wasm command) to confirm the removed `local_fs` cfg split didn't leave wasm-only gaps.
