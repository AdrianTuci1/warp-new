...
5. Triggering the `input:insert_network_logging_workflow` keybinding ("Show Octomus network log") opens the same pane with the same layout behavior.
...
14. The previously available "Tail Octomus network log" workflow is removed from the command palette and workflow list. Users will no longer find a workflow that runs `tail -f` on a network log file.
