...
Add a user setting named `Use latest user prompt as conversation title in tab names` that controls which conversation text Octomus shows in vertical tabs for agent conversations.
...
The desired outcome is that Octomus's default tab-name behavior uses stable, title-like conversation labels, while users who prefer prompt-based labels can explicitly opt into using the latest prompt.
...
Octomus exposes a setting in the AI settings area, near settings that affect agent input or agent conversation behavior.
...
- Oz agent conversations, including local agent conversations and cloud agent conversations shown in Octomus.
- Plugin-backed third-party CLI agent sessions, such as supported Claude Code, Codex, or similar sessions where Octomus receives structured session metadata from the agent plugin.
...
- Non-terminal panes such as code panes, notebooks, workflows, settings, files, or Octomus Drive objects.
...
- If the preferred text type is missing, Octomus falls back to the other text type when available.
- If both title-like metadata and prompt metadata are missing, Octomus uses the existing terminal fallback chain.
...
If Octomus normally syncs comparable AI display preferences across devices, this setting should follow that same sync behavior. If comparable AI display preferences are local-only, this setting should follow that local-only behavior instead.
...
- **Persistence**: Toggle the setting, restart Octomus, and verify the setting value and vertical-tabs behavior are preserved.
