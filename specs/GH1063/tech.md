# TECH.md — Rename Oz to Octomus Agent in settings and onboarding

Issue: https://github.com/warpdotdev/octomus-external/issues/1063
Product spec: `specs/GH1063/product.md`

## Context

This is a rename of the in-app agent from "Oz" to "Octomus Agent" across user-facing
...
1. `app/src/settings_view/mod.rs`
   ...
   - In the `Display` impl, the `WarpAgent` arm writes `"Octomus Agent"`.
   - In the `FromStr` impl, accept both `"Oz"` (backward-compat legacy name)
     and `"Octomus Agent"` as parseable forms that map to
...
2. `app/src/settings_view/ai_page.rs`
   ...
   - Replace all remaining user-visible description strings that referenced
     ...
     allowlist/denylist descriptions, Rules description, Octomus Drive context
...
3. `crates/onboarding/src/slides/agent_slide.rs`
   - `render_header`: change paragraph text from `"Customize your Agent, Oz"`
     to `"Customize your Octomus Agent"`. Keep font size, weight, layout, and
     surrounding subtitle unchanged.
   - `render_disable_oz_section`: change checkbox label from `"Disable Oz"` to
     `"Disable Octomus Agent"`. Keep styling, spacing, `disable_oz_mouse` state
...
- `cargo fmt` and `cargo clippy --workspace --all-targets --all-features --tests
  -- -D warnings` must pass (per `OCTOMUS.md` PR workflow).
- `cargo nextest run -p warp_app --no-fail-fast` or the relevant subset covering
  ...
  `SettingsSection::WarpAgent.to_string() == "Octomus Agent"`. The `FromStr` test
  covers both `"Oz"` and `"Octomus Agent"` resolving to
...
- Behavior #1, #2, #3, #9: manually open the settings UI and confirm the
  sidebar entry reads "Octomus Agent", the subpage renders unchanged content, the
  heading above the global toggle reads "Octomus Agent", and the "Oz Cloud API
...
- Behavior #7: search within the settings modal using each of
  `"octomus agent"`, `"ai"`, `"agent"`, `"oz"` (should still reach the subpage) and
  `"oz cloud"` (should reach the cloud subpage only).
- Behavior #8: confirm both `"Oz"` and `"Octomus Agent"` resolve to
...
- Behavior #12: toggle the `OpenWarpNewSettingsModes` feature flag and confirm
  the disable row only appears when enabled and always reads "Disable Octomus
...
- Risk: external deep links or persisted telemetry strings reference `"Oz"` and
  break. Mitigation: `FromStr` accepts both `"Oz"` and `"Octomus Agent"` mapping to
...
- Risk: stale comments inside `agent_slide.rs` that still reference "Disable Oz"
  ...
  comments describing them are acceptable to leave as-is per `OCTOMUS.md`.
