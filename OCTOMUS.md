...
### Build and Run
- `cargo run` - Build and run Octomus locally
- `cargo bundle --bin octomus` - Bundle the main app

### Running with local octomus-server
To connect Octomus client to a local octomus-server instance:
...
- `cargo nextest run -p octomus_completer --features v2` - Run completer tests with v2 features
...
- `./script/run-clang-format.py -r --extensions 'c,h,cpp,m' ./crates/octomusui/src/ ./app/src/` - Format C/C++/Obj-C code
...
**Core Libraries**:
- `crates/octomus_core/` - Core utilities and platform abstractions
- `crates/editor/` - Text editing functionality
- `crates/octomusui/` and `crates/octomusui_core/` - Custom UI framework
...
5. **Cloud Sync**: Objects can be synchronized across devices via Octomus Drive

### Development Guidelines

**Workspace Structure**:
- This is a Cargo workspace with 60+ member crates
- Main binary is in `app/`, UI framework in `crates/octomusui/`
...
**GraphQL**:
- Schema and client code generation from `crates/octomus_graphql_schema/api/schema.graphql`
- TypeScript types generated for frontend integration

### Feature Flags

Octomus uses compile-time feature flags with a small runtime plumbing layer.

How to add a feature flag:
- Add a new variant to `octomus_core/src/features.rs` in the `FeatureFlag` enum
