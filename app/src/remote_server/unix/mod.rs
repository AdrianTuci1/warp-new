...
pub(crate) fn launch_daemon(identity_key: &str, ctx: &mut octomusui::AppContext) {
    let socket_path = proxy::socket_path(identity_key);
    let pid_path = proxy::pid_path(identity_key);
    ...
    let timing_data =
        octomus_core::interval_timer::IntervalTimer::handle(ctx).update(ctx, |timer, _| {
...
pub(super) async fn handle_daemon_connection(
    ...
    spawner: octomusui::ModelSpawner<ServerModel>,
