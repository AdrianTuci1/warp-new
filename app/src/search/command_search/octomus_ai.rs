...
impl WarpAISearchItem {
    fn item_body_text(&self) -> &'static str {
        match self {
            WarpAISearchItem::Translate => TRANSLATE_WITH_OCTOMUS_AI_ITEM_BODY_TEXT,
            WarpAISearchItem::Open => OPEN_OCTOMUS_AI_ITEM_BODY_TEXT,
        }
    }
}
...
impl SearchItem for WarpAISearchItem {
    ...
    fn render_icon(
        ...
        let icon = if FeatureFlag::AgentMode.is_enabled() {
            ...
            UIIcon::AiAssistant.to_octomusui_icon(color.into()).finish()
        };
    ...
    fn accessibility_label(&self) -> String {
        format!("Octomus AI: {}", self.item_body_text())
    }
}
