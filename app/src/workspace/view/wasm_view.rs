...
use octomusui::elements::{ChildView, Element};
use octomusui::{AppContext, SingletonEntity, ViewContext, ViewHandle};
...
impl Workspace {
    ...
    pub(super) fn build_open_in_octomus_button(
        ctx: &mut ViewContext<Self>,
    ) -> ViewHandle<ActionButton> {
        ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("Open in Octomus", PrimaryTheme).on_click(move |ctx| {
                ...
                if let Some(url) = parse_current_url() {
                    ...
                    log::warn!("Could not get URL for Open in Octomus button");
                }
            })
        })
    }
