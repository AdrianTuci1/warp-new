...
use octomusui::text_layout::{TextAlignment, TextStyle};
...
impl GlobalSearchView {
    ...
    pub fn set_root_directories(&mut self, roots: Vec<PathBuf>, _ctx: &mut ViewContext<Self>) {
        ...
        self.search_roots = octomus_util::path::group_roots_by_common_ancestor(&roots).roots;
    ...
    fn render_file_header(
        ...
        Hoverable::new(file_mouse_state, move |mouse_state| {
            ...
            let chevron_icon = chevron_icon_enum
                .to_octomusui_icon(ThemeFill::from(chevron_color))
                .finish();
            ...
            let file_icon = match icon_from_file_path {
                ImageOrIcon::Icon(icon) => {
                    icon.to_octomusui_icon(ThemeFill::from(icon_color)).finish()
                }
    ...
    fn render_directory_header_from_entry(
        ...
        Hoverable::new(mouse_state, move |mouse_state| {
            ...
            let folder_icon = Icon::Folder
                .to_octomusui_icon(ThemeFill::from(icon_color))
                .finish();
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if self.capped_matches {
            let alert_icon = ConstrainedBox::new(
                Icon::AlertTriangle
                    .to_octomusui_icon(ThemeFill::Solid(
    ...
    fn render_zero_state(
        ...
        let main_column = Flex::column()
            ...
            .with_child(
                Container::new(
                    ConstrainedBox::new(
                        icon.to_octomusui_icon(ThemeFill::Solid(internal_colors::neutral_6(theme)))
                            .finish(),
                    )
                    .with_width(24.)
                    .with_height(24.)
