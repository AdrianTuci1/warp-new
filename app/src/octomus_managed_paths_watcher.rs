...
/// Duration between filesystem watch events for the Octomus managed paths watcher, in milliseconds.
#[cfg(not(target_family = "wasm"))]
...
pub(crate) fn ensure_warp_watch_roots_exist() {
    ...
    if let Err(err) = fs::create_dir_all(&data_dir) {
        log::warn!(
            "Failed to create Octomus data directory {}: {err}",
            data_dir.display()
        );
    }

    let config_local_dir = octomus_core::paths::config_local_dir();
    if config_local_dir != data_dir {
        if let Err(err) = fs::create_dir_all(&config_local_dir) {
            log::warn!(
                "Failed to create Octomus config directory {}: {err}",
...
pub(crate) fn warp_home_config_dir() -> Option<PathBuf> {
    octomus_core::paths::warp_home_config_dir()
}
...
pub(crate) fn warp_home_mcp_config_file_path() -> Option<PathBuf> {
    octomus_core::paths::warp_home_mcp_config_file_path()
}
                ...
                (warp_home_config_dir(), warp_home_mcp_config_file_path())
            {
                        ...
                        || !warp_home_config_dir.starts_with(&config_local_dir))
                {
                    ...
                    Self::register_path(
                        ...
                        "Octomus home MCP config directory",
                    );
                }
            }
        }
...
#[cfg(test)]
#[path = "octomus_managed_paths_watcher_tests.rs"]
mod tests;
