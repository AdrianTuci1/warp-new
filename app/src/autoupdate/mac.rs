...
use octomus_core::macos::get_bundle_path;
...
fn old_executable_file_path() -> PathBuf {
    octomus_core::paths::state_dir().join(OLD_EXECUTABLE_FILE_NAME)
}
...
pub(super) fn relaunch() -> Result<()> {
    ...
    // autoupdate process.
    launch_command.push(format!(" --args {}", octomus_cli::finish_update_flag()));
    ...
    // We need to make sure that the current Octomus process is no longer running
...
pub async fn cleanup_all_except(preserve_update_id: Option<&str>) {
    let mut autoupdate_dir = octomus_core::paths::cache_dir();
...
/// Determines if the user needs authorization in order to update Octomus.
...
/// * Octomus can create files in the directory
/// * Octomus can modify the permissions of created files
...
/// team identifier matches Octomus's team identifier.
async fn verify_code_signature(component: &str, path: &Path) -> Result<()> {
    ...
    let codesign_verify_output = Command::new("/usr/bin/codesign")
        ...
        .arg(format!(
            ...
            octomus_core::macos::APPLE_TEAM_ID
        ))
        .arg(path)
        .output()
        .await?;
...
async fn apply_update(channel: Channel, version_info: &VersionInfo, update_id: &str) -> Result<()> {
    ...
    if async_fs::metadata(executable_temp_file.as_path())
        ...
        .is_ok()
    {
        // If we performed this process already but didn't relaunch Octomus, the old executable will
...
fn get_download_dir(update_id: &str) -> PathBuf {
    let mut dir = octomus_core::paths::cache_dir();
    dir.push("autoupdate");
    dir.push(update_id);
    dir
}
...
async fn mount_dmg(dmg_dir: &Path, update_id: &str) -> Result<PathBuf> {
    ...
    // -nobrowse: Do not show the Octomus DMG in Finder or similar apps.
    // -noautoopen: Do not open the Octomus DMG in Finder.
