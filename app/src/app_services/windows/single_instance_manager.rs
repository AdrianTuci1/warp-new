...
pub(super) fn uri_named_pipe_name() -> String {
    format!("Octomus{:?}_URI_CHANNEL", ChannelState::channel())
}
...
fn try_create_mutex() -> Result<Option<MutexHandle>, Error> {
    ...
    // `script/windows/windows-installer.iss`, which the installer uses to detect whether Octomus is
    // running.
    let name = format!("Local\\Octomus{:?}_SingleInstance", ChannelState::channel())
        .encode_utf16()
...
/// A singleton model that is responsible for ensuring there is only one instance of Octomus running.
...
impl SingleInstanceManager {
    /// Attempts to upgrade the current Octomus instance to the "main" instance (i.e. the one that
    ...
    /// Returns whether or not this process should be treated as the main instance of Octomus.
    ///
