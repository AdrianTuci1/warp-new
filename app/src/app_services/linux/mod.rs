...
use octomusui::r#async::executor::BackgroundTask;
...
pub fn pass_startup_args_to_existing_instance(
    ...
    octomusui::r#async::block_on(async {
...
pub enum StartupArgsForwardingError {
    /// There's no instance of Octomus already running.
...
impl From<zbus::fdo::Error> for StartupArgsForwardingError {
    fn from(value: zbus::fdo::Error) -> Self {
        ...
        // registering a service, so this really means that Octomus is not already
        // running.
...
// A D-Bus client for connecting to an already-running instance of Octomus and
// invoking org.freedesktop.Application IPC methods.
#[proxy(
    interface = "org.freedesktop.Application",
    default_service = "dev.octomus.WarpLocal",
    default_path = "/dev/octomus/WarpLocal",
    gen_blocking = false
)]
...
impl DBusServiceHost {
    fn new(ctx: &mut octomusui::ModelContext<Self>) -> Self {
    ...
    fn terminate(&mut self) {
        if let Some(server_task) = self.server_task.take() {
            ...
            report_if_error!(octomusui::r#async::block_on(server_task));
        }
    }
...
impl octomusui::Entity for DBusServiceHost {
    type Event = ();
}
