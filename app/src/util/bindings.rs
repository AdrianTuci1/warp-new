...
lazy_static! {
    ...
    /// know about particular Octomus specific actions, so it deals with all actions
    /// as plain isizes.  Within Octomus though we want to deal with them as the enum type.
    ...
    pub static ref MAC_PTY_NON_COMPLIANT_ACTIONS: HashSet<&'static str> = HashSet::from_iter(["terminal:octomusify_subshell", "terminal:open_block_list_context_menu_via_keybinding"]);
...
pub fn custom_tag_to_keystroke(custom: CustomTag) -> Option<Keystroke> {
    match custom.into() {
        ...
        CustomAction::ToggleOctomusDrive => {
            if OperatingSystem::get().is_mac() {
                Keystroke::parse("cmd-\\").ok()
            } else {
                Keystroke::parse("ctrl-shift-|").ok()
...
/// [`octomusui::keymap::FixedBinding`] or [`EditableBinding`].
...
pub fn cmd_or_ctrl_shift(key: &str) -> String {
    if OperatingSystem::get().is_mac() {
        ...
        let key = if Keystroke::is_valid_special_key(key) {
            ...
            // https://github.com/warpdotdev/octomus-internal/blob/c225b8cedd94fdba33e957cf1efb99d84768d193/ui/src/keymap.rs#L637/
