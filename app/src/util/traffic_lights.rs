...
//! depends on the platform. The Octomus app must use this information to avoid rendering UI elements
//! underneath them.
...
/// Data the Octomus app needs to avoid rendering anything below the traffic lights.
#[derive(Clone, Debug)]
...
impl TrafficLightData {
    ...
    /// by octomusui. However, native traffic light buttons (e.g. on macOS) don't scale with zoom, so
    ...
    fn render_windows_close_button_icon(icon_color: ColorU) -> Box<dyn Element> {
        ConstrainedBox::new(
            IconComponent::X
                .to_octomusui_icon(theme::Fill::Solid(icon_color))
                .finish(),
        )
        .with_height(16.)
        .with_width(16.)
