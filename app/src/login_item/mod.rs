//! Login-item registration — makes Octomus start automatically when the user
...
/// Reconciles whether Octomus is registered to launch at login with the user's
/// current preference.
///
/// Respects the existing `app_added_as_login_item` bookkeeping so a user who
/// removed Octomus from their OS's startup UI isn't silently re-added — the
...
/// auto-launch `target/debug/{octomus,openwarp,...}` at sign-in.
#[cfg(any(target_os = "macos", target_os = "windows"))]
