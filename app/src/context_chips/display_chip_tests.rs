...
fn test_github_pr_display_text_from_url() {
    assert_eq!(
        github_pr_display_text_from_url("https://github.com/octomus/octomus/pull/123"),
...
fn test_github_pr_display_text_from_url_rejects_non_pr_urls() {
    assert_eq!(
        github_pr_display_text_from_url("https://github.com/octomus/octomus/issues/123"),
        None
    );
    assert_eq!(
        github_pr_display_text_from_url("https://github.com/octomus/octomus/pull/not-a-number"),
        None
    );
}
...
fn test_format_git_branch_command_reports_missing_linked_worktree_path() {
    ...
    assert_eq!(
        format_git_branch_command(&value),
        "echo 'Branch '\\''feature-a'\\'' is already checked out in another worktree, but Octomus couldn'\\''t find its path.'"
    );
}
