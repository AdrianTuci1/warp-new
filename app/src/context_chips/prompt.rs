...
use octomusui::{Entity, GetSingletonModelHandle, ModelContext, SingletonEntity, UpdateModel};
...
impl Prompt {
    ...
    /// Reset to the default Octomus prompt.
    ...
    /// Whether same line prompt is enabled for Octomus prompt.
    pub fn same_line_prompt_enabled(&self) -> bool {
        self.config.same_line_prompt_enabled
    }

    /// The separator to be used for the Octomus prompt.
    pub fn separator(&self) -> WarpPromptSeparator {
...
impl PromptConfiguration {
    /// The default Octomus prompt, synthesized from legacy prompt settings.
