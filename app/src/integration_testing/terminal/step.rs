...
use octomusui::{async_assert, Event, SingletonEntity};
...
fn switch_to_terminal_mode_and_type_command(
    ...
    command: String,
) -> impl Fn(&mut octomusui::App, octomusui::WindowId) -> Event + 'static {
    move |app, window_id| {
...
fn restore_ai_mode_if_needed(
    ...
    pane_idx: usize,
    was_ai_mode: Arc<AtomicBool>,
) -> impl Fn(&mut octomusui::App, octomusui::WindowId) + 'static {
    move |app, window_id| {
