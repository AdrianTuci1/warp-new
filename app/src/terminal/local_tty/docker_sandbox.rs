...
//!
//! This module owns everything specific to running a Octomus shell inside a
...
/// Prefix for generated container names: `octomus-sandbox-<id>`.
const DOCKER_SANDBOX_NAME_PREFIX: &str = "octomus-sandbox";

/// Root directory on the host under which Docker-sandbox scratch files
/// (bash init scripts, empty workspace mount points) live.
///
/// Lives under the Octomus per-user cache directory rather than `/tmp` so:
...
fn docker_sandbox_host_root() -> PathBuf {
    octomus_core::paths::cache_dir().join("docker-sandbox")
}

/// Resolves the absolute path to the `sbx` CLI binary using the Octomus
/// process's `PATH`.
///
/// Octomus's process `PATH` is minimal and often misses user-shell-installed
/// tools (e.g. homebrew on Apple Silicon when Octomus is launched from Finder,
...
///
/// Each instance carries a unique `sandbox_id` so multiple Octomus panes can run
...
impl DockerSandboxShellStarter {
    ...
    /// Host directory where Octomus writes this sandbox's bash init script.
