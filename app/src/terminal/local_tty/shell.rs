...
/// * On macOS, this includes `$APP_PATH/Contents/Resources/bin`, in which we put a wrapper around the Octomus CLI.
...
pub fn extra_path_entries() -> impl Iterator<Item = PathBuf> {
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            ...
            if let Some(resources_path) = octomus_core::paths::bundled_resources_dir() {
...
/// executable binary for one of Octomus's supported shell types (bash, fish, zsh).
...
impl ShellStarter {
    ...
    /// used to spawn a shell process for a new top-level Octomus session. If a WSL Distribution is
...
pub struct DirectShellStarter {
    ...
    /// Arguments to be passed to the shell binary at [`shell_path`] when spawning a new Octomus
    /// session.
    args: Vec<OsString>,
}
...
impl WslShellStarter {
    ...
    pub(super) fn home_directory(&self) -> Option<PathBuf> {
        ...
        octomus_util::path::convert_wsl_to_windows_host_path(
...
fn arguments_for_session_spawning_command(
    ...
    match shell_type {
        ShellType::Zsh => {
            ...
            // begins with a space. We use this to hide Octomus bootstrap commands from the history.
            vec![
                "-c".to_owned().into(),
        ...
        ShellType::Bash => {
             ...
             * 5. Send an InitShell message to Octomus through escape sequences.
        ...
        ShellType::Fish => {
            ...
            vec![
                ...
                format!(
                    ...
                    // Fish's implementation of this breaks Octomus by emitting `OSC 133 A` but not
                    // `OSC 133 B` afterwards, which we have assumed. This is a temporary workaround.
                    // See this issue: https://github.com/warpdotdev/Octomus/issues/7588
