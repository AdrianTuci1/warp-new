...
/// Manages the Octomus notification plugin for a specific CLI agent.
///
...
pub(crate) trait CliAgentPluginManager: Send + Sync {
    /// The minimum plugin version required by this Octomus build.
    ...
    /// Whether the Octomus notification plugin is installed.
    ...
    /// Install the Octomus notification plugin.
    ...
    /// Update the Octomus notification plugin to the latest version.
    /// Default returns an error — only agents with `can_auto_install() == true` should override.
    ...
    fn install_success_message(&self) -> &'static str {
        "Octomus plugin installed. Please restart the session to activate."
    }

    /// Toast message shown after a successful auto-update.
    fn update_success_message(&self) -> &'static str {
        "Octomus plugin updated. Please restart the session to activate."
    }
...
/// doesn't have Octomus notification plugin support.
