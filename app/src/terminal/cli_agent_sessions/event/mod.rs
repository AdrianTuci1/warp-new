...
pub const CLI_AGENT_NOTIFICATION_SENTINEL: &str = "octomus://cli-agent";
...
/// How a CLI agent event reached Octomus.
...
/// The current CLI agent protocol version this build of Octomus supports.
        ...
        None => {
            log::error!(
                "Received CLI agent event with unsupported schema version \
                 {version}. The CLI agent plugin or Octomus may need to be updated."
            );
            None
        }
    }
