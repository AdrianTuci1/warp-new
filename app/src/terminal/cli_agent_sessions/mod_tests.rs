...
fn parse_stop_notification() {
    ...
    let notif = parse_event(Some("octomus://cli-agent"), body).unwrap();
...
fn returns_none_for_invalid_json() {
    assert!(parse_event(Some("octomus://cli-agent"), "not json").is_none());
}
...
fn rejects_unsupported_schema_version() {
    ...
    assert!(parse_event(Some("octomus://cli-agent"), body).is_none());
}
...
fn parse_auggie_stop_notification() {
    // Mirrors what the community auggie-octomus plugin emits on the Stop hook.
