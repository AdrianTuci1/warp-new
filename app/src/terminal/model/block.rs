...
use octomusui::record_trace_event;
use octomusui::units::{IntoLines, Lines};
...
pub enum BlockState {
    ...
    /// This block holds static content and is programmatically added to the blocklist by Octomus. An
...
impl Block {
    ...
    /// line prompt, we render on the same line for PS1, but not for Octomus prompt!
    ...
    /// when the shell echoes the command bytes that Octomus wrote to the PTY.
    ...
    pub fn prompt_and_command_grid_offset(&self) -> Lines {
        if self.header_grid.honor_ps1() {
            ...
            // Grid is drawn below custom Octomus prompt in finished blocks.
            self.block_banner_height()
                + self.padding_top()
                + self.prompt_height()
                + self.command_padding_top()
    ...
    /// In the case of combined grid: for Octomus prompt, this includes the height of both the Octomus prompt
    ...
    pub fn prompt_and_command_height(&self) -> Lines {
        if !self.ready_to_render() || self.should_hide_command_grid {
            ...
            // Handle the case of Octomus built-in prompt with combined grid.
    ...
    pub fn top_level_command(&self, sessions: &Sessions) -> Option<String> {
        ...
        let command = octomus_completer::parsers::simple::top_level_command(
        ...
        session
            ...
            .and_then(|s| octomus_completer::parsers::simple::top_level_command(s, escape_char))
    ...
    fn wakeup_after_delay(&self) {
        ...
        self.background_executor
            .spawn(async move {
                octomusui::r#async::Timer::after(std::time::Duration::from_millis(delay_ms)).await;
...
impl ansi::Handler for Block {
    ...
    fn linefeed(&mut self) -> ScrollDelta {
        ...
        // prompt, but this is not needed in Octomus due to us visually separating
        // blocks.
        match self.header_grid.receiving_chars_for_prompt {
