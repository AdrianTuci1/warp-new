...
fn new_command_executor_for_local_tty_session(
    ...
    if FeatureFlag::SSHTmuxWrapper.is_enabled()
        ...
        // We don't allow nested tmux octomusification, so if our parent session is already warified using
    ...
    // `sbx exec octomus-sandbox-<id> -- sh -c "<cmd>"` (analogous to
    ...
    match &session_info.session_type {
        ...
        _ => {
            if *should_force_disable_in_band_generators {
                ...
                // codepath is only accessible via a user defaults command that a Octomus
