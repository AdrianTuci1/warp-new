...
/// Indicates that the following JSON-encoded message is hex-encoded for Octomus's lifecycle hooks.
...
/// Indicates that the following JSON-encoded message is unencoded for Octomus's lifecycle hooks.
/// In DCS, it is used as the final char in the DCS start sequence.
/// In OSC, it is used as the first parameter.
pub(super) const UNENCODED_JSON_MARKER: char = 'f';

/// Indicates that the following message is a ANSI-C quoted message for receiving Octomus's lifecycle
/// hooks via key-value pairs.
/// In OSC< it is used as the first parameter.
pub(super) const UNENCODED_KV_MARKER: char = 'k';

/// Enum representing all possible JSON payloads for Octomus's DCS's.
...
pub(super) enum DProtoHook {
    ...
    RemoteOctomusificationIsUnavailable {
        // If a value is provided, it's suggesting a way to install TMUX on the remote.
        value: OctomusificationUnavailableReason,
    },
    SshTmuxInstaller {
        value: String,
    },
...
impl DProtoHook {
    pub fn name(&self) -> &'static str {
        match self {
            ...
            DProtoHook::RemoteOctomusificationIsUnavailable { .. } => {
                "RemoteOctomusificationIsUnavailable"
            }
...
/// The reason that octomusification was not available when the user tried
/// to octomusify.
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all(serialize = "snake_case"))]
pub enum OctomusificationUnavailableReason {
    TmuxFailed,
...
/// Received from the pty after the shell has finished executing Octomus's
/// bootstrap script.
...
/// Received from the pty when Octomus's SSH wrapper is executed, prior to
...
/// created; if the session is for a subshell, this triggers Octomus's bootstrap process.
...
/// The binding is triggered when Octomus writes the `ESC-i` escape sequence to the pty.
/// Octomus usually does this after a block completes, to collect any typeahead
...
/// `logout`, Ctrl-D on an empty prompt, etc.). Lets the Octomus client drop
