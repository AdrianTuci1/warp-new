...
use octomus_terminal::model::ansi::control_sequence_parameters::*;
...
pub trait Handler {
    ...
    /// Callback for the Octomus CommandFinished hook.
    ...
    /// Callback for the Octomus precmd hook.
    ...
    /// Callback for the Octomus preexec hook.
    fn preexec(&mut self, _data: PreexecValue) {}

    /// Callback for the Octomus bootstrapped hook - called once when the shell is
    /// bootstrapped
    fn bootstrapped(&mut self, _data: BootstrappedValue) {}

    /// Callback for the Octomus pre-interactive SSH session hook - called once
    ...
    /// Callback for the Octomus ssh hook - called once after successfully connecting to
    /// an SSH server
    ...
    /// Callback for the Octomus exit-shell hook — emitted by the remote shell right
    /// before it exits. Gives the Octomus client a chance to drop per-session
    ...
    /// input buffer (the reporting is itself triggered by Octomus).
    ...
    /// created. If the session is for a subshell, this should triggers Octomus's bootstrap process.
    ...
    /// These allow external programs to trigger notifications in Octomus.
