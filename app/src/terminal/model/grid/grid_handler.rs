...
use octomus_core::semantic_selection::{SemanticSelection, SMART_SELECT_MATCH_WINDOW_LIMIT};
use octomus_core::{safe_assert, safe_assert_eq};
use octomus_terminal::model::grid::{CellType, FlatStorage};
pub use octomus_terminal::model::TermMode;
...
/// grid that needs to be kept in sync with Octomus's grids. We do this via clearing the ConPTY
/// grid before Octomus starts populating a new grid.
///
...
impl GridHandler {
    ...
    pub(super) fn input_at_cursor(&mut self, text: &str) {
        use octomus_terminal::model::VisiblePoint;
