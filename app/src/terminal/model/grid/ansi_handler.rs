...
pub(super) struct State {
    ...
    /// correctly. Notably, Zsh does NOT support this well in bracketed paste mode (which we use for all Octomus
...
impl ansi::Handler for GridHandler {
    ...
    fn identify_terminal<W: std::io::Write>(&mut self, writer: &mut W, intermediate: Option<char>) {
        match intermediate {
            ...
            Some('>') => {
                ...
                // Octomus, rather than passing the higher `Pv` value, we decided to use one of the
    ...
    fn set_color(&mut self, _: usize, _: octomusui::color::ColorU) {
