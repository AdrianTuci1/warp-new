...
fn test_mac_octomusification_script_size() {
    App::test(ASSETS, |mut app| async move {
        initialize_app(&mut app);

        app.read(|ctx| {
            assert_script_is_short_enough_mac(
                &begin_octomusify_ssh_session_command(ctx),
            ...
            assert_script_is_short_enough_mac(
                ...
                "install_tmux_and_octomusify_brew.sh",
                false,
            );
            assert_script_is_short_enough_mac(
                &get_script(
                ...
                "fish/install_tmux_and_octomusify_brew.sh",
                false,
            );

            assert_script_is_short_enough_mac(
                &octomusify_ssh_session_command("Darwin", ShellType::Zsh, ctx)
                    .expect("Should get Darwin zsh script"),
                "zsh octomusify",
                true,
            );
            assert_script_is_short_enough_mac(
                &octomusify_ssh_session_command("Darwin", ShellType::Bash, ctx)
                    .expect("Should get Darwin bash script"),
                "bash octomusify",
                true,
            );
            assert_script_is_short_enough_mac(
                &octomusify_ssh_session_command("Darwin", ShellType::Fish, ctx)
