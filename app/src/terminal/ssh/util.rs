...
/// For some [`SshOctomusifyCommand`]s, we do not support parsing
...
impl SshOctomusifyCommand {
    pub fn matches(command: &str) -> Option<SshOctomusifyCommand> {
        let command = if let Some(suffix) = command.strip_prefix("command ") {
            suffix
        } else {
            command
        ...
        if INTERACTIVE_SSH.is_match(command) {
            Some(SshOctomusifyCommand::Ssh)
        } else if GCLOUD_REGEX.is_match(command) {
            Some(SshOctomusifyCommand::SshLike(SshLikeCommand::Gcloud))
        } else if ELASTIC_BEANSTALK_REGEX.is_match(command) {
            Some(SshOctomusifyCommand::SshLike(
...
pub fn parse_interactive_ssh_command(command: &str) -> Option<InteractiveSshCommand> {
    match SshOctomusifyCommand::matches(command) {
        Some(SshOctomusifyCommand::Ssh) => InteractiveSshCommand::parse_ssh_command(command),
        Some(SshOctomusifyCommand::SshLike(SshLikeCommand::Gcloud)) => {
            Some(InteractiveSshCommand::default())
        }
        Some(SshOctomusifyCommand::SshLike(SshLikeCommand::ElasticBeanstalk)) => {
            Some(InteractiveSshCommand::default())
        }
        Some(SshOctomusifyCommand::SshLike(SshLikeCommand::DigitalOceanDroplet)) => {
            Some(InteractiveSshCommand::default())
        }
        None => None,
    }
