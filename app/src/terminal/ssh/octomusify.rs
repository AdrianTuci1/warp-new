...
#[derive(Debug, Clone)]
pub enum SshOctomusifyBlockEvent {
    OctomusifySession,
    Cancel,
    Interrupt,
}
...
pub enum SshOctomusifyBlockAction {
    Interrupt,
    Focus,
}

pub struct SshOctomusifyBlock {
...
pub fn init(app: &mut AppContext) {
    ...
    app.register_fixed_bindings([FixedBinding::new(
        "ctrl-c",
        SshOctomusifyBlockAction::Interrupt,
        id!(SshOctomusifyBlock::ui_name()),
    )]);
}

impl SshOctomusifyBlock {
...
impl Entity for SshOctomusifyBlock {
    type Event = SshOctomusifyBlockEvent;
}

impl SshOctomusifyBlock {
    fn render_title_ui(&self, theme: &WarpTheme, appearance: &Appearance) -> Box<dyn Element> {
        let icon = Icon::new(UiIcon::Octomus.into(), theme.active_ui_detail());
        octomusify::render::header_row("Octomusifying SSH Session...", icon, theme, appearance)
    }
}

pub fn octomusify_description(
    app: &AppContext,
    ...
    let description = FormattedText::new(vec![FormattedTextLine::Line(vec![
        FormattedTextFragment::plain_text(
            "Bring Octomus's features to your remote session. Blocks, full text editing, auto-complete, Oz, and more. "
        ),
...
impl View for SshOctomusifyBlock {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        .on_click(|ctx, _, _| {
            ctx.dispatch_typed_action(SshOctomusifyBlockAction::Focus);
        })
        .finish()
    }
}

impl TypedActionView for SshOctomusifyBlock {
    type Action = SshOctomusifyBlockAction;

    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            SshOctomusifyBlockAction::Interrupt => {
                ctx.emit(SshOctomusifyBlockEvent::Interrupt);
            }
            SshOctomusifyBlockAction::Focus => {
                self.focus(ctx);
            }
        }
    }
...
/// Convert the begin_octomusify_ssh_session script into a string.
pub fn begin_octomusify_ssh_session_command(app: &AppContext) -> String {
    ...
    match AssetCache::as_ref(app).load_asset::<String>(asset) {
        ...
        _ => panic!("ssh begin octomusify script should be available as a string"),
    }
}

/// Convert the octomusify_ssh_session script into a string.
pub fn octomusify_ssh_session_command(
    uname: &str,
    ...
    let asset = match (uname, shell_type) {
        ...
        ("Darwin", ShellType::Zsh | ShellType::Bash) => {
            bundled_asset!("ssh/bash_zsh/octomusify_ssh_session_mac.sh")
        }
        // Mac scripts must be less than 1020 characters due to macOS 15+ pty issue
        ("Darwin", ShellType::Fish) => bundled_asset!("ssh/fish/octomusify_ssh_session_mac.sh"),
        (_, ShellType::Zsh | ShellType::Bash) => {
            bundled_asset!("ssh/bash_zsh/octomusify_ssh_session.sh")
        }
        (_, ShellType::Fish) => bundled_asset!("ssh/fish/octomusify_ssh_session.sh"),
    ...
    match AssetCache::as_ref(app).load_asset::<String>(asset) {
        ...
        _ => panic!("ssh octomusify script should be available as a string"),
    }
}
#[cfg(test)]
#[path = "octomusify_tests.rs"]
mod tests;
