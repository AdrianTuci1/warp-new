...
impl OnboardingAgenticSuggestionsBlock {
    pub fn new(
        ...
        let matrix_save_directory = themes_dir()
            .into_os_string()
            .into_string()
            .unwrap_or("the Octomus themes directory.".to_string());
        ...
        let agent_suggestions = vec![
            ...
            (
                AgenticSuggestionsContent {
                    ...
                    prompt: format!("First check if {matrix_save_directory} exists, and create this path if it doesn't already exist. Then create a matrix theme for my Octomus terminal without a background image field, following exact YAML structure on the octomus website without any extra or missing fields. Call it matrix.yaml and save it in the directory we previously created. Once you've verified that the theme is correct and ready to be applied, let me know by only saying 'The matrix theme is now available at <path>.'."),
        ...
        let mut content = Flex::row().with_child(
            ConstrainedBox::new(
                self.agent_suggestions[index]
                    .0
                    .icon
                    .to_octomusui_icon(main_text_color)
                    .finish(),
            )
            ...
            .with_child({
                let button = Hoverable::new(mouse_state_handle.clone(), |state| {
                    ...
                    let background_color = if is_disabled {
                        ...
                        octomus_core::ui::theme::Fill::Solid(internal_colors::neutral_1(theme))
                    };
    ...
    fn render_text(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        const WELCOME_TEXT_LINE_ONE: &str = "Welcome to Octomus!";
                ...
                Flex::row()
                    ...
                    .with_child(
                        Container::new(
                            ConstrainedBox::new(
                                UIIcon::Icon::Loading
                                    .to_octomusui_icon(ai_brand_color(appearance.theme()).into())
                                    .finish(),
                            )
