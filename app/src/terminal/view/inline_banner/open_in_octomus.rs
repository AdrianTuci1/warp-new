...
use octomusui::{Element, EntityId};
...
use crate::terminal::view::open_in_octomus::OpenablePath;
...
/// Given an openable file, format a file-specific title for the Open in Octomus banner.
fn file_title_text(openable_path: &OpenablePath) -> String {
    match openable_path.file_type {
        OpenableFileType::Markdown => {
            "Did you know that Octomus can directly display Markdown files?".to_string()
        }
            ...
            cfg_if::cfg_if! {
                if #[cfg(not(target_family = "wasm"))] {
                    ...
                    match language.as_ref().map(|language| language.display_name()) {
                        Some(display_name) => {
                            format!("Did you know that Octomus can directly edit {display_name} files?")
                        }
                        None => "Did you know that Octomus can directly edit code?".to_string(),
                    }
                } else {
                    // The `languages` crate is not available on WASM, so use a fallback message.
                    "Did you know that Octomus can directly edit code?".to_string()
                }
            }
        }
    }
...
pub fn render_open_in_octomus_banner(
    state: &OpenInWarpBannerState,
    view_id: EntityId,
    appearance: &Appearance,
) -> Box<dyn Element> {
    let button_text = match state.target.file_type {
        OpenableFileType::Markdown => "View in Octomus",
        OpenableFileType::Code | OpenableFileType::Text => "Edit in Octomus",
    };
    ...
    let open_button = InlineBannerTextButton {
        ...
        position_id: Some(format!("open_in_octomus_banner_button_{view_id}")),
