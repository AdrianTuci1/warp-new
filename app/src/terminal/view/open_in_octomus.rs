...
use octomus_completer::parsers::simple::all_parsed_commands;
...
#[cfg(test)]
#[path = "open_in_octomus_tests.rs"]
mod tests;

const LEARN_MORE_MARKDOWN_URL: &str =
    "https://docs.octomus.dev/terminal/more-features/markdown-viewer";
const LEARN_MORE_CODE_URL: &str = "https://docs.octomus.dev/code/overview#built-in-code-editor";

/// A path to a file that can be opened in Octomus, along with its type.
...
impl TerminalView {
    pub(super) fn maybe_suggest_open_in_octomus(
        &mut self,
                ...
                move |view, maybe_match, ctx| {
                    if let Some(openable_path) = maybe_match {
                        if matches!(
                            ...
                            view.suggest_open_in_octomus(openable_path, session, ctx);
                        }
                    }
                },
            );
    ...
    pub fn is_open_in_octomus_banner_open(&self) -> bool {
        self.inline_banners_state.open_in_octomus_banner.is_some()
    }

    fn close_open_in_octomus_banner(&mut self, banner_id: usize) {
        self.model
            .lock()
            .block_list_mut()
    ...
    fn open_in_octomus_banner_type_dismissed(
        &self,
        ...
        match file_type {
            OpenableFileType::Markdown => {
                *general_settings.open_in_octomus_banner_dismissed_for_markdown
            }
            OpenableFileType::Code | OpenableFileType::Text => {
                *general_settings.open_in_octomus_banner_dismissed_for_code_and_text
            }
        }
    }

    /// Insert a suggestion banner for opening the file `openable_path`, originating from
    /// `session`, in a Octomus pane.
    fn suggest_open_in_octomus(
        &mut self,
        openable_path: OpenablePath,
        session: Arc<Session>,
        ctx: &mut ViewContext<Self>,
    ) {
        if self.open_in_octomus_banner_type_dismissed(openable_path.file_type, ctx) {
            return;
        }

        // We only show a banner for the most recent command.
        if let Some(prev_state) = &self.inline_banners_state.open_in_octomus_banner {
            self.close_open_in_octomus_banner(prev_state.id);
        }

        let banner_id = self.inline_banners_state.next_banner_id();
        self.inline_banners_state.open_in_octomus_banner = Some(OpenInWarpBannerState::new(
            banner_id,
            openable_path,
            session,
        ));
    ...
    pub fn handle_open_in_octomus_banner_action(
        &mut self,
        ...
        match action {
            ...
            OpenInWarpBannerAction::LearnMore => {
                if let Some(banner_state) = &self.inline_banners_state.open_in_octomus_banner {
                        ...
                        OpenableFileType::Markdown => {
                            GeneralSettings::handle(ctx).update(ctx, |settings, ctx| {
                                report_if_error!(settings
                                    .open_in_octomus_banner_dismissed_for_markdown
                                    .set_value(true, ctx));
                            });
                        }
                        OpenableFileType::Code | OpenableFileType::Text => {
                            GeneralSettings::handle(ctx).update(ctx, |settings, ctx| {
                                report_if_error!(settings
                                    .open_in_octomus_banner_dismissed_for_code_and_text
                                    .set_value(true, ctx));
                            });
                        }
                    }
    ...
    pub fn open_in_octomus_banner_accessibility_content(
        &self,
        action: OpenInWarpBannerAction,
    ) -> ActionAccessibilityContent {
        match action {
            OpenInWarpBannerAction::OpenFile => {
                match &self.inline_banners_state.open_in_octomus_banner {
                    Some(banner_state) => {
...
/// Examines `command` for a file openable in Octomus, returning the resolved path and type if found.
        ...
        if let Some(positionals) = positionals {
            for arg in positionals.iter() {
                ...
                if async_fs::metadata(&resolved).await.is_ok() {
                    // We've found a file that exists and can be opened in Octomus.
