...
maybe_define_setting!(AddedSubshellCommands, group: OctomusifySettings, {
    type: Vec<String>,
    default: Vec::new(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "octomusify.subshells.added_subshell_commands",
    description: "Additional regex patterns for commands that should be recognized as subshells.",
});

maybe_define_setting!(SubshellCommandsDenylist, group: OctomusifySettings, {
    type: Vec<String>,
    default: Vec::new(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "octomusify.subshells.subshell_commands_denylist",
    description: "Commands that should not trigger the subshell octomusification prompt.",
});

maybe_define_setting!(SshHostsDenylist, group: OctomusifySettings, {
    type: Vec<String>,
    default: Vec::new(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "octomusify.ssh.ssh_hosts_denylist",
    description: "SSH hosts that should not trigger the octomusification prompt.",
});

maybe_define_setting!(EnableSshOctomusification, group: OctomusifySettings, {
    type: bool,
    default: true,
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "octomusify.ssh.enable_ssh_octomusification",
    description: "Whether to enable Octomus features in SSH sessions.",
});

maybe_define_setting!(UseSshTmuxWrapper, group: OctomusifySettings, {
    type: bool,
    default: false,
    supported_platforms: SupportedPlatforms::OR(SupportedPlatforms::MAC.into(), SupportedPlatforms::LINUX.into()),
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "octomusify.ssh.use_ssh_tmux_wrapper",
    description: "Whether to use a tmux-based wrapper for SSH octomusification.",
});

/// Controls how Octomus handles the SSH extension (remote server binary) when connecting
...
pub enum SshExtensionInstallMode {
    ...
    AlwaysInstall,
    /// Never install; fall back to legacy octomusification.
    NeverInstall,
}

maybe_define_setting!(SshExtensionInstallModeSetting, group: OctomusifySettings, {
    type: SshExtensionInstallMode,
    default: SshExtensionInstallMode::default(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "octomusify.ssh.ssh_extension_install_mode",
    description: "Controls SSH extension installation behavior.",
});
...
pub struct OctomusifySettings {
    ...
    /// A list of commands that we shouldn't attempt to octomusify. These can be added either b/c the
    /// "don't ask again" button was clicked in the trigger banner, or it was added explicitly on
    /// the Octomusify settings page. This represents the raw, serialized value.
    ...
    /// A list of hosts that we shouldn't attempt to octomusify. This supports regex.
    /// These can be added either b/c the "don't ask again" button was clicked in the trigger banner,
    /// or it was added explicitly on the Octomusify settings page.
    /// While this could live in the `SshSettings` group, the custom processing shared with the other
    /// subshell logic better justifies it living in the `OctomusifySettings` group.
    ...
    /// This setting controls whether we should ever octomusify ssh sessions.
    pub enable_ssh_octomusification: EnableSshOctomusification,

    /// This setting controls whether we should prompt the user to octomusify an ssh session using the
...
lazy_static! {
    ...
    static ref SUBSHELL_COMMAND_REGEXES: Vec<Regex> = vec![
        ...
        // https://github.com/warpdotdev/Octomus/issues/2736
...
impl OctomusifySettings {
    fn new_from_storage(ctx: &mut ModelContext<Self>) -> Self {
        ...
        Self {
            ...
            ssh_hosts_denylist,
            enable_ssh_octomusification: EnableSshOctomusification::new_from_storage(ctx),
    ...
    pub fn new_with_defaults(_ctx: &mut ModelContext<Self>) -> Self {
        ...
        Self {
            ...
            parsed_ssh_hosts_denylist: Self::parse_ssh_hosts_denylist(&ssh_hosts_denylist),
            ssh_hosts_denylist,
            enable_ssh_octomusification: EnableSshOctomusification::new(None),
    ...
    pub fn register(ctx: &mut AppContext) {
        ...
        handle.clone().update(ctx, |_, ctx| {
            ctx.subscribe_to_model(&handle, |me, event, _| match event {
                OctomusifySettingsChangedEvent::AddedSubshellCommands { .. } => {
                    me.parsed_added_subshell_commands =
                        Self::parse_added_subshell_commands(&me.added_subshell_commands)
                }
                OctomusifySettingsChangedEvent::SubshellCommandsDenylist { .. } => {
                    me.parsed_subshell_command_denylist =
                        Self::parse_subshell_command_denylist(&me.subshell_command_denylist)
                }
                OctomusifySettingsChangedEvent::SshHostsDenylist { .. } => {
                    me.parsed_ssh_hosts_denylist =
                        Self::parse_ssh_hosts_denylist(&me.ssh_hosts_denylist)
                }
                OctomusifySettingsChangedEvent::EnableSshOctomusification { .. } => {}
                OctomusifySettingsChangedEvent::UseSshTmuxWrapper { .. } => {}
                OctomusifySettingsChangedEvent::SshExtensionInstallModeSetting { .. } => {}
            })
        });
...
/// define_settings_group!(OctomusifySettings). Since we didn't use that macro we define it manually
/// here. It's the event emitted by the setter methods when a setting value changes.
pub enum OctomusifySettingsChangedEvent {
    AddedSubshellCommands {
...
impl Entity for OctomusifySettings {
    type Event = OctomusifySettingsChangedEvent;
}

impl SingletonEntity for OctomusifySettings {}
...
impl OctomusifySettings {
    ...
    pub fn is_compatible_subshell_command(&self, command: &str, shell_family: ShellFamily) -> bool {
        ...
        if !self.use_ssh_tmux_wrapper.value()
            && SshOctomusifyCommand::matches(command)
        ...
        // While in-band generators are our best option for octomusifying ssh sessions from powershell, hard-code
        // the octomusify subshell banner to show up.
    ...
    pub fn add_subshell_command(&mut self, command_to_add: &str, ctx: &mut ModelContext<Self>) {
        ...
        // care of emitting the OctomusifySettingsChangedEvent::AddedSubshellCommands event to keep
    ...
    /// Check if the user has asked us to remember a command and avoid asking to octomusify a subshell.
