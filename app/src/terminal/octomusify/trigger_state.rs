...
pub enum SshBlockState {
    Octomusifying {
        handle: ViewHandle<SshOctomusifyBlock>,
    },
    OctomusifySuccess {
        handle: ViewHandle<OctomusifySuccessBlock>,
    },
    InstallTmux {
...
impl SshBlockState {
    ...
    pub fn get_block_view_id(&self) -> EntityId {
        match self {
            SshBlockState::Octomusifying { handle, .. } => handle.id(),
            SshBlockState::OctomusifySuccess { handle, .. } => handle.id(),
    ...
    pub fn collapse_script(&mut self, ctx: &mut ViewContext<TerminalView>) -> bool {
        match self {
            ...
            SshBlockState::Octomusifying { .. } => false,
            SshBlockState::OctomusifySuccess { .. } => false,
    ...
    pub fn focus(&mut self, ctx: &mut ViewContext<TerminalView>) {
        match self {
            SshBlockState::Octomusifying { handle } => {
                handle.update(ctx, |block, ctx| block.focus(ctx));
            }
            SshBlockState::OctomusifySuccess { .. } => {}
            SshBlockState::InstallTmux { handle } => {
                handle.update(ctx, |block, ctx| block.focus(ctx));
            }
    ...
    pub fn get_system_details(&self, app: &AppContext) -> Option<SystemDetails> {
        match self {
            ...
            SshBlockState::Octomusifying { .. } => None,
            SshBlockState::OctomusifySuccess { .. } => None,
    ...
    pub fn on_warpified_session_complete(
        ...
        match self {
            SshBlockState::InstallTmux { .. } | SshBlockState::Octomusifying { .. } => {
                let block_id = self.get_block_view_id();
                return Some(block_id);
            }
            SshBlockState::OctomusifySuccess { handle } => {
...
/// Temporary state used to trigger Octomusification.
#[derive(Default)]
struct OctomusifyTriggerState {
    block_id: Option<BlockId>,

    /// Lets us abort an attempt to auto octomusify if the subshell command
    /// hasn't completed.
    auto_octomusify_abort_handle: Option<SpawnedFutureHandle>,
    ...
    /// The command which may trigger ssh Octomusification
    pending_command: Option<String>,
    /// The Host which may trigger ssh Octomusification
    pending_octomusify_ssh_host: Option<String>,

    /// Which, if any, SSH block is currently added to the blocklist.
    ssh_block_state: Option<SshBlockState>,

    ssh_octomusify_timeout_handle: Option<SpawnedFutureHandle>,
...
#[derive(Default)]
pub struct OctomusifyState {
    session_id: Option<SessionId>,

    pending_state: Option<OctomusifyTriggerState>,
...
impl OctomusifyState {
    ...
    pub fn add_auto_octomusify_abort_handle(&mut self, spawned_future_handle: SpawnedFutureHandle) {
        let pending_state = self.pending_state.get_or_insert_with(Default::default);
        pending_state.auto_octomusify_abort_handle = Some(spawned_future_handle);
    }

    pub fn abort_auto_octomusify(&mut self) {
        if let Some(abort_handle) = self
            .pending_state
            .as_mut()
            .and_then(|state| state.auto_octomusify_abort_handle.take())
        {
            abort_handle.abort();
        };
    }
    ...
    pub fn abort_ssh_octomusify_timeout(&mut self) {
        self.replace_timeout_id();
        if let Some(handle) = self
            .pending_state
            .as_mut()
            .and_then(|state| state.ssh_octomusify_timeout_handle.take())
        {
            handle.abort();
        };
    }
    ...
    pub fn get_pending_ssh_host(&self) -> Option<String> {
        self.pending_state
            .as_ref()
            .and_then(|state: &OctomusifyTriggerState| state.pending_octomusify_ssh_host.clone())
    }

    pub fn get_pending_ssh_command(&self) -> Option<String> {
        self.pending_state
            .as_ref()
            .and_then(|state: &OctomusifyTriggerState| state.pending_command.clone())
    }

    pub fn take_pending_ssh_host(&mut self) -> Option<String> {
        self.pending_state
            .as_mut()
            .and_then(|state: &mut OctomusifyTriggerState| state.pending_octomusify_ssh_host.take())
    }

    pub fn clear_pending_ssh_host(&mut self) {
        if let Some(ref mut pending_state) = self.pending_state.as_mut() {
            pending_state.pending_octomusify_ssh_host = None;
        }
    }
    ...
    pub fn set_pending_ssh_host(&mut self, command: String, ssh_host: Option<String>) {
        ...
        pending_state.pending_octomusify_ssh_host = ssh_host;
    }
    ...
    /// and we have a Octomusify Success block.
    fn on_warpified_session_complete(
        &mut self,
        state: OctomusifyTriggerState,
        ctx: &mut ViewContext<TerminalView>,
    ) -> Option<EntityId> {
    ...
    pub fn on_octomusify_start(&mut self, active_session_id: Option<SessionId>) {
        self.session_id = active_session_id;
    }

    /// Called whenever a block is completed, to determine whether a Warpified session
    /// has been completed.
    pub fn get_completed_octomusify_session_id(
        &mut self,
