...
pub const SSH_DOCS_URL: &str = "https://docs.octomus.dev/terminal/octomusify/ssh";
pub const SUBSHELL_DOCS_URL: &str = "https://docs.octomus.dev/terminal/octomusify/subshells";
...
/// UI helper to render the ssh command that caused the octomusification prompt.
...
/// Renders a "Never Octomusify this host" link or nothing.
pub fn render_never_octomusify_ssh_link(
    ssh_host: &Option<String>,
    app: &AppContext,
    appearance: &Appearance,
    mouse_state_handle: MouseStateHandle,
    on_never_octomusify: fn(&mut EventContext<'_>, ssh_host: String),
) -> Option<Box<dyn Element>> {
    let Some(ssh_host) = ssh_host else {
        return None;
    };

    let settings = OctomusifySettings::handle(app);
    if settings.as_ref(app).is_ssh_host_denylisted(ssh_host) {
        // Should only happen if user manually attempts to Octomusify a denylisted host.
        return None;
    }
    ...
    let link = appearance
        .ui_builder()
        .link(
            "Never Octomusify this host".into(),
            None,
            Some(Box::new({
                let ssh_host = ssh_host.clone();
                move |ctx| on_never_octomusify(ctx, ssh_host.to_owned())
            })),
            mouse_state_handle,
        )
        .soft_wrap(false)
...
pub fn render_subshell_flag_pole(
    max_height: f32,
    fill: impl Into<octomusui::elements::Fill>,
) -> Box<dyn Element> {
