...
use octomusui::{Entity, ModelContext, ModelHandle};
...
/// This prevents Octomus sending an escape sequence to an arbitrary running program.
///
...
impl LineEditorStatus {
    ...
    fn mark_line_editor_active_after_delay(
        ...
        let abort_handle = ctx.spawn_abortable(
            async move { octomusui::r#async::Timer::after(delay).await },
            |me, _, ctx| {
