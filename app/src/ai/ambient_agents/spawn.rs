...
fn poll_run_until_joinable_session(
    ...
    async_stream::stream! {
        ...
        let mut timeout_timer = FutureExt::fuse(match timeout {
            Some(d) => octomusui::r#async::Timer::after(d),
            None => octomusui::r#async::Timer::never(),
        });
        ...
        loop {
            let mut poll_timer = FutureExt::fuse(octomusui::r#async::Timer::after(TASK_STATUS_POLL_INTERVAL));
