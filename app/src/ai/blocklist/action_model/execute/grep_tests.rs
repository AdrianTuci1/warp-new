...
fn test_create_redacted_grep_error_event() {
    ...
    let queries = vec![
        ...
        "query with octomus-server-staging.firebaseapp.com secret".to_string(),
    ];
    let path = "path/to/file/with/octomus-server-staging.firebaseapp.com/secret".to_string();
    let shell_type = Some(ShellType::Bash);
    let working_directory = Some("/users/test/octomus-server-staging.firebaseapp.com".to_string());
    let absolute_path =
        "/absolute/path/with/octomus-server-staging.firebaseapp.com/secret".to_string();
    let error = GrepError::new("Error message".to_string())
        .with_command("grep octomus-server-staging.firebaseapp.com".to_string())
        .with_output("Output with octomus-server-staging.firebaseapp.com".to_string());
    ...
    } = event
    {
        ...
        assert!(!redacted_queries[1].contains("octomus-server-staging.firebaseapp.com"));
        assert!(redacted_queries[1].contains("*****"));

        assert!(!redacted_path.contains("octomus-server-staging.firebaseapp.com"));
        assert!(redacted_path.contains("*****"));

        assert!(!redacted_working_directory.contains("octomus-server-staging.firebaseapp.com"));
        assert!(redacted_working_directory.contains("*****"));

        assert!(!redacted_absolute_path.contains("octomus-server-staging.firebaseapp.com"));
        assert!(redacted_absolute_path.contains("*****"));

        assert!(!redacted_command.contains("octomus-server-staging.firebaseapp.com"));
        assert!(redacted_command.contains("*****"));

        assert!(!redacted_output.contains("octomus-server-staging.firebaseapp.com"));
