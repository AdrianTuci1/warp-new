...
impl FileBasedMCPManager {
    ...
    /// - For `MCPProvider::Octomus`: the logical root for `~/.octomus*/.mcp.json`.
        ...
        self.file_based_servers_by_root
            ...
            .any(|(root_path, provider_map)| {
                provider_map.iter().any(|(provider, hashes)| {
                    ...
                    match provider {
                        MCPProvider::Octomus => Self::is_global_warp_root(root_path),
    ...
    /// Octomus config (`~/.octomus/.mcp.json`). Global Octomus servers always auto-spawn.
    ...
    fn maybe_autostart_file_based_servers(
        ...
        // - Global Octomus: always auto-spawn.
        // - Global non-Octomus: auto-spawn iff the toggle is on.
    ...
    fn handle_file_based_mcp_enabled_change(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // - Global Octomus servers always spawn regardless of the toggle.
        ...
        if !AISettings::as_ref(ctx).is_file_based_mcp_enabled(ctx) {
            ...
            // Toggle on: spawn global third-party servers (global Octomus servers are
    ...
    /// exception: global Octomus installs are discovered in `~/.octomus*/`, which
    /// isn't a useful cwd for spawned processes, so they are remapped to the
    /// home directory instead.
    /// - Project-scoped installations: the repo root.
    /// - Global installations (`~/.octomus/.mcp.json`, `~/.claude.json`, etc.): the
    ///   home directory.
    ///
    ...
    pub fn spawn_root_for_installation(&self, uuid: Uuid) -> Option<PathBuf> {
        ...
        // Global Octomus installs live under `~/.octomus*/`, which is internal Octomus
        // state rather than a meaningful working directory. Map them to the
        // home dir so all global installs (Octomus and third-party) share a
        // consistent cwd.
...
enum FileBasedMCPServerType {
    /// A file-based MCP server detected from Octomus's global managed config.
    GlobalWarp,
    /// A file-based MCP server detected from a non-Octomus provider's global user config.
    GlobalThirdParty,
