...
/// updates to Octomus's secure storage via a channel.
///
...
/// runtime token auto-refreshes are written back to Octomus's secure storage.
///
...
pub async fn make_authenticated_client(
        ...
        .and_then(ChannelState::mcp_oauth_provider_by_issuer)
    {
        ...
        oauth_state
            .start_authorization(&[], &redirect_uri, Some("Octomus"))
            .await?;
        oauth_state
    };
...
impl TemplatableMCPServerManager {
    ...
    pub fn handle_oauth_callback(&mut self, url: &Url) -> anyhow::Result<()> {
        ...
        octomusui::r#async::block_on(server_info.oauth_result_tx.send(result)).map_err(|_| {
...
pub(crate) fn load_credentials_from_secure_storage<T: DeserializeOwned + Default>(
    ...
    app.secure_storage()
        .read_value(key)
        .inspect_err(|err| {
            if !matches!(err, octomusui_extras::secure_storage::Error::NotFound) {
