//! Commands to interact with ambient agents on Octomus's platform.
                    ...
                    match event_result {
                        Ok(event) => match event {
                            ...
                            AmbientAgentEvent::TimedOut => {
                                ...
                                println!("Agent session with run ID {task_id_str} is not ready after {}s. Check for a sharing link in the ambient agent management panel. See https://docs.octomus.dev/agent-platform/cloud-agents/managing-cloud-agents for details.", TASK_STATUS_POLLING_DURATION.as_secs());
                            }
                        },
                        Err(err) => {
                            return Err(err);
...
fn task_id_from_oz_run_id_env() -> anyhow::Result<Option<AmbientAgentTaskId>> {
    match std::env::var(octomus_cli::OZ_RUN_ID_ENV) {
...
impl octomusui::Entity for AmbientAgentRunner {
    type Event = ();
}
