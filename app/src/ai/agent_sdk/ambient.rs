                    ...
                    match event_result {
                        Ok(event) => match event {
                            ...
                            AmbientAgentEvent::TimedOut => {
                                ...
                                println!("Agent session with run ID {task_id_str} is not ready after {}s. Check for a sharing link in the Octomus App agent management panel.", TASK_STATUS_POLLING_DURATION.as_secs());
                            }
                        },
                        Err(err) => {
                            return Err(err);
