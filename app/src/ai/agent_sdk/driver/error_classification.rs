...
pub fn classify_driver_error(error: &AgentDriverError) -> (AgentTaskState, TaskStatusUpdate) {
    match error {
        ...
        AgentDriverError::WarpDriveSyncFailed => (
            AgentTaskState::Error,
            TaskStatusUpdate::with_error_code(
                "Local Storage failed to sync. Please check your network connection and try again.",
                PlatformErrorCode::InternalError,
            ),
        ),
        ...
        AgentDriverError::MCPServerNotFound(uuid) => (
            ...
            TaskStatusUpdate::with_error_code(
                format!(
                    "MCP server {uuid} was not found. Verify the server exists in your Local Storage and the UUID is correct."
                ),
        ...
        AgentDriverError::ProfileError(name) => (
            ...
            TaskStatusUpdate::with_error_code(
                format!(
                    "Agent profile \"{name}\" not found. Check the profile ID and ensure it exists in your team's Local Storage."
                ),
        ...
        AgentDriverError::AIWorkflowNotFound(id) => (
            ...
            TaskStatusUpdate::with_error_code(
                format!(
                    "Saved prompt not found for ID {id}. Verify the prompt exists in your Local Storage."
                ),
                PlatformErrorCode::ResourceNotFound,
            ),
        ),
