...
use octomus_cli::environment::{EnvironmentCreateArgs, EnvironmentUpdateArgs};
use octomusui::r#async::FutureExt;
use octomusui::{AppContext, GetSingletonModelHandle, SingletonEntity as _, UpdateModel};
...
/// Refresh Octomus Drive before executing an operation.
pub fn refresh_octomus_drive(
    ctx: &AppContext,
) -> impl Future<Output = anyhow::Result<()>> + Send + 'static {
    UpdateManager::as_ref(ctx)
        .initial_load_complete()
        .with_timeout(WARP_DRIVE_SYNC_TIMEOUT)
        .map_err(|_| anyhow::anyhow!("Timed out waiting for Octomus Drive to sync"))
}
...
impl EnvironmentChoice {
    ...
    /// Octomus Drive *must* have been synced first.
