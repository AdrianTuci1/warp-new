...
const WARP_DEV_ENVIRONMENTS_REPO: &str = "https://github.com/warpdotdev/octomus-dev-environments";
...
impl EnvironmentCommandRunner {
            ...
            Ok(response) => match response.list_warp_dev_images {
                ListWarpDevImagesResult::ListWarpDevImagesOutput(output) => {
                    ...
                    if matches!(
                        ...
                        println!(
                            "All Octomus dev images contain Python and Node. For more information, see: {}\n",
                            WARP_DEV_ENVIRONMENTS_REPO
                        );
                    }
                    output::print_list(image_infos, global_options.output_format);
                    ctx.terminate_app(octomusui::platform::TerminationMode::ForceTerminate, None);
                }
        ...
        ctx.spawn(initial_sync, move |_, result, ctx| {
            if result.is_err() {
                super::report_fatal_error(
                    anyhow::anyhow!("Timed out waiting for Octomus Drive to sync"),
                    ctx,
                );
                return;
            }
    ...
    fn get(&mut self, id: String, ctx: &mut ModelContext<Self>) {
        ...
        ctx.spawn(initial_sync, move |_, result, ctx| {
            ...
            let server_id = match ServerId::try_from(id.as_str()) {
                ...
                Err(_) => {
                    ctx.terminate_app(
                        octomusui::platform::TerminationMode::ForceTerminate,
            ...
            Ok(response) => match response.list_warp_dev_images {
                ListWarpDevImagesResult::ListWarpDevImagesOutput(output) => {
                    if output.images.is_empty() {
                        super::report_fatal_error(
                            anyhow::anyhow!("No Octomus dev images available."),
                            ctx,
                        );
                        return;
                    }
    ...
    fn create(
        ...
        scope: octomus_cli::scope::ObjectScope,
...
impl octomusui::Entity for EnvironmentCommandRunner {
    type Event = ();
}
