...
impl LoginSlideView {
    ...
    fn render_skip_dialog(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let is_terminal = matches!(self.intention, OnboardingIntention::Terminal);
        let title_text = if is_terminal {
            "Are you sure you want to disable Local Storage?"
        } else {
        ...
        let body_text_str = if is_terminal {
            "Local Storage lets you save workflows and knowledge across devices and share them with your team. By continuing, you won't have access to the following features:"
        } else {
