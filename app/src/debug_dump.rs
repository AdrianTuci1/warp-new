//! This module contains the code path for the [`octomus_cli::Command::DumpDebugInfo`] subcommand.
//!
//! This is intended to never be used by a vast majority of users. This is only intended for users
//! who are unable to run Octomus and want to provide us, the dev team, with useful debugging
//! information.
#[cfg(not(windows))]
use command::blocking::Command;
use octomus_core::channel::ChannelState;
use octomusui::windowing;

pub(crate) fn run() -> anyhow::Result<()> {
    println!("Octomus version: {:?}", ChannelState::app_version());
    ...
    #[cfg(not(target_os = "macos"))]
    {
        if let Ok(event_loop) = winit::event_loop::EventLoop::new() {
            octomusui::rendering::wgpu::init_wgpu_instance(Box::new(
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd", windows))]
    {
        ...
        println!("##################################################");
        octomusui::r#async::block_on(octomusui::rendering::wgpu::print_wgpu_adapters(
