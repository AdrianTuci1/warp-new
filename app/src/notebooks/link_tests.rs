...
fn test_resolve_bare_url() {
    App::test((), |mut app| async move {
        ...
        assert_eq!(
            resolve(&app, &links, "octomus.dev").await,
            url("http://octomus.dev")
        );
        assert_eq!(
...
fn test_resolve_valid_url() {
    App::test((), |mut app| async move {
        ...
        assert_eq!(
            resolve(&app, &links, "https://octomus.dev").await,
            url("https://octomus.dev")
        );
        assert_eq!(
            resolve(&app, &links, "mailto:test@octomus.dev").await,
            url("mailto:test@octomus.dev")
        );
    });
}
