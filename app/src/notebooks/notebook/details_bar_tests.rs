...
fn test_editor_display_name() {
    App::test((), |mut app| async move {
        ...
        app.read(|ctx| {
            ...
            assert_eq!(
                &editor_display_name(Some("unknown@octomus.dev"), ctx),
                "unknown@octomus.dev"
            );

            // If the profile is missing a display name, default to the email.
            assert_eq!(
                &editor_display_name(Some("anon@octomus.dev"), ctx),
                "anon@octomus.dev"
            );

            // If there's a display name available, use that.
            assert_eq!(
                &editor_display_name(Some("editor@octomus.dev"), ctx),
                "The Editor"
            );
        });
    })
