...
mod non_mac {
    ...
    /// Sets Octomus as the default terminal
...
pub struct DefaultTerminal {
    /// Whether the OS will treat Octomus as the default app for scripts/executables.
...
impl DefaultTerminal {
    ...
    /// This is an OS-level setting. Unlike most other settings, where Octomus is the source-of-truth
    /// for the value of the setting, it can be changed outside of Octomus. We monitor if it gets
    /// changed externally by checking when Octomus is focused.
    ...
    /// This is a one-way operation. Once we set the default terminal to Octomus, we can't really
    ...
    pub fn make_warp_default(&mut self, ctx: &mut ModelContext<Self>) {
        if let Err(e) = set_warp_as_default_terminal() {
            log::error!("Error setting Octomus as default terminal: {e:#}");
        } else {
