...
fn filter_repository_update_by_prefix_keeps_only_matching_paths() {
    let skills_dir = PathBuf::from("/tmp/.octomus-local/skills");
    let other_dir = PathBuf::from("/tmp/.octomus-local/worktrees/repo");
...
fn filter_repository_update_by_prefix_converts_cross_boundary_moves() {
    let skills_dir = PathBuf::from("/tmp/.octomus-local/skills");
    let skill_file = skills_dir.join("deploy").join("SKILL.md");
    let ignored_file = PathBuf::from("/tmp/.octomus-local/worktrees/repo/SKILL.md");
