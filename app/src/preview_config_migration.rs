...
//! directory (`~/.octomus-preview`) on macOS.
//!
//! Historically, Stable and Preview shared `~/.octomus` on macOS. To give
//! Preview its own directory without breaking existing users, this migration
//! symlinks each top-level entry from `~/.octomus` into `~/.octomus-preview` on
...
use octomus_core::paths::{data_dir, WARP_CONFIG_DIR};
...
/// Migrates Preview's config directory from the shared `.octomus` location to
/// `.octomus-preview` by creating symlinks from each top-level entry in `.octomus`
...
/// - `~/.octomus-preview` already exists.
/// - `~/.octomus` does not exist.
