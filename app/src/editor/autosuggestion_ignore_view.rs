...
impl View for AutosuggestionIgnore {
    ...
    fn render(&self, ctx: &octomusui::AppContext) -> Box<dyn Element> {
        ...
        let line_height_ratio = appearance
            .line_height_ratio()
            .min(octomusui::elements::DEFAULT_UI_LINE_HEIGHT_RATIO);
        ...
        Hoverable::new(self.autosuggestion_ignore_mouse_handle.clone(), |state| {
            ...
            let close_icon = Container::new(
                ConstrainedBox::new(Icon::X.to_octomusui_icon(Fill::Solid(icon_color)).finish())
