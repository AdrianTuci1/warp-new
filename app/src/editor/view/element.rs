...
use octomusui::text_selection_utils::{
...
impl EditorElement {
        ...
        for cursor in cursors {
            ...
            if !is_local {
                    ...
                    remote_selections_data.get_mut(&cursor.replica_id)
                {
                    ...
                    ctx.scene.start_layer(octomusui::ClipBounds::None);
    ...
    pub fn with_input_editor_icons(
        ...
        let cycle_next_command_hint = if self.should_show_cycle_next_command_hint(is_cycling, ctx) {
            ...
            Some(
                self.render_cycle_next_command_hint(octomus_core::ui::theme::Fill::Solid(
    ...
    fn render_cycle_next_command_hint(&self, color: Fill) -> Box<dyn Element + 'static> {
        ...
        Flex::row()
            ...
            .with_children([
                Container::new(
                    ConstrainedBox::new(Icon::ArrowDown.to_octomusui_icon(color).finish())
