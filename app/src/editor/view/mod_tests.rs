    ...
    App::test((), |mut app| async move {
        ...
        app.add_window(WindowStyle::NotStealFocus, |ctx| {
            let mut editor = EditorView::new_with_base_text(
                "word ~/.octomus/themes/foo-bar.yaml thing",
                Default::default(),
                ctx,
            );
            editor.select_word(&DisplayPoint::new(0, 8), ctx);
            assert_eq!(editor.selected_text(ctx), "~/.octomus/themes/foo-bar.yaml");
            editor
        });
    });
}
...
fn test_smart_select_with_drag() {
    App::test((), |mut app| async move {
        ...
        app.add_window(WindowStyle::NotStealFocus, |ctx| {
            let mut editor = EditorView::new_with_base_text(
                "word ~/.octomus/themes/foo-bar.yaml andy@octomus.dev",
                Default::default(),
                ctx,
            );
            editor.select_word(&DisplayPoint::new(0, 8), ctx);
            ...
            assert_eq!(
                ...
                "~/.octomus/themes/foo-bar.yaml andy"
            );
            editor
        });
    });
...
fn test_autosuggestions() -> Result<()> {
    use octomusui::text_layout::LayoutCache;
...
fn test_autocomplete_symbols() {
    ...
    App::test((), |mut app| async move {
        ...
        app.add_window(WindowStyle::NotStealFocus, |ctx| {
            let mut editor =
                EditorView::new_with_base_text("word octomus word", Default::default(), ctx);
            ...
            editor.add_next_occurrence(ctx);
            editor.user_insert("[", ctx);
            assert_eq!(editor.buffer_text(ctx), "[word] octomus [word]");
    ...
    App::test((), |mut app| async move {
        ...
        app.add_window(WindowStyle::NotStealFocus, |ctx| {
            ...
            let mut editor = EditorView::new_with_base_text("word octomus word", options, ctx);
...
fn test_paste_clipboard_with_text_only_should_paste_text_normally() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            let clipboard_content = octomusui::clipboard::ClipboardContent {
...
fn test_paste_clipboard_with_image_only_should_switch_to_agent_mode() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            let png_image = octomusui::clipboard::ImageData {
...
fn test_paste_clipboard_with_unsupported_image_and_text_should_show_error() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            let bmp_image = octomusui::clipboard::ImageData {
    ...
    App::test((), |mut app| async move {
        ...
        view.update(&mut app, |view, ctx| {
            ...
            view.set_drag_drop_path_transformer(Some(Box::new(
                octomus_util::path::convert_windows_path_to_wsl,
            )));
            view.drag_and_drop_files(&paths(), ctx);
        ...
        view.update(&mut app, |view, ctx| {
            view.clear_buffer(ctx);
            view.set_drag_drop_path_transformer(Some(Box::new(
                octomus_util::path::convert_windows_path_to_msys2,
            )));
            view.drag_and_drop_files(&paths(), ctx);
