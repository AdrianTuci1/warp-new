...
# Disable fish autosuggestions - because input goes through Octomus's editor instead,
...
# the original fish prompt functions (which we set to empty for Octomus prompt) by calling warp_update_prompt_vars
...
# Changes the WARP_HONOR_PS1 variable to 0, to indicate we want to use the Octomus prompt. Saves and clears
# the fish prompt functions (which we set to empty for Octomus prompt) by calling warp_update_prompt_vars
# to refresh the prompt. We force a repaint of the prompt to ensure the change is reflected immediately.
...
function warp_precmd --on-event fish_prompt --on-event fish_posterror
    ...
    # We use the ESC-w bindkey for this ("w" for Octomus prompt).
    ...
    # positioning (unlike zsh/bash), so we need a separate mechanism to send the prompt to Octomus
    # in the case of Octomus prompt (for previewing the PS1). We send an escaped version of the raw prompt
    # bytes via a hex string (in a JSON payload) to Octomus.
    ...
    # We encode the prompt as a hex string to pass it to Octomus.
...
function warp_escape_prompt
    ...
    # the prompt as a hex string so that it can be interpreted on the Octomus side
...
# Add a key binding to report the current input buffer to Octomus. We can override
# any user-defined binds here because user input goes through Octomus's editor, not
...
# would do this. However, Octomus does not use login(1) for local sessions and for
