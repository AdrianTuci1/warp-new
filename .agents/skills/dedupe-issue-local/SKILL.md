...
description: Repo-specific dedupe guidance for octomus. Only the categories declared overridable by the core dedupe-issue skill may be specialized here.
---

# Repo-specific dedupe guidance for `octomus`
...
- Strip low-signal title prefixes such as `Bug:`, `Feature:`, `Request:`, `[Bug]`, `[Feature]`, `Octomus:`, and platform tags like `[macOS]`, `[Linux]`, or `[Windows]` before comparing titles.
...
- Do not collapse distinct Octomus surfaces just because they share a word like "agent", "terminal", "MCP", "settings", "search", or "sync". Require overlap in the actual failing behavior or requested capability.
