...
description: Add a new feature flag to gate code changes in the Octomus codebase.
---

# add-feature-flag

Add a new feature flag to gate code changes in the Octomus codebase.

## Overview

Feature flags in Octomus are compile-time flags that allow features to be selectively enabled for different channels (e.g.: Dev, Stable). They use a small runtime plumbing layer that checks if a flag is enabled.
...
Add a new variant to the `FeatureFlag` enum in `octomus_core/src/features.rs`:
...
When ready to enable the feature for all Octomus Stable users, add it to the `default` array in `app/Cargo.toml`:
