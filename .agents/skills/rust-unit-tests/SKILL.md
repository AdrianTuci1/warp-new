...
description: Write, improve, and run Rust unit tests in the octomus Rust codebase.
---

# Rust Unit Tests in octomus
...
- Prefer `octomusui::App::test` for deterministic unit tests around views/models.
- Initialize app models once, then mutate via `update` and assert via `read`.

```rust
use octomusui::App;
// In app crate tests prefer `crate::test_util::...`; from other crates use `octomus::test_util::...`.
use octomus::test_util::{terminal::initialize_app_for_terminal_view, add_window_with_terminal};
...
```rust
use octomus::features::FeatureFlag; // or `use crate::features::FeatureFlag;` inside the app crate
