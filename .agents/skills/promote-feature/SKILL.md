...
description: Promote a feature-flagged feature to Dogfood, Preview, or Stable in the Octomus codebase. Use when a feature behind a FeatureFlag is ready to roll out to a broader audience, including wiring up the compile-time/runtime bridge and deferring flag cleanup safely.
---
...
- **Runtime** (`octomus_core/src/features.rs`): `DOGFOOD_FLAGS`, `PREVIEW_FLAGS`, `RELEASE_FLAGS` — enabled per-channel at startup.
...
Add the flag to `DOGFOOD_FLAGS` in `octomus_core/src/features.rs`:
...
1. Add to `PREVIEW_FLAGS` in `octomus_core/src/features.rs`.
...
Prefer this over adding to `RELEASE_FLAGS` (see comment at `octomus_core/src/features.rs:787-790`). It compiles the feature into all builds and enables a one-line rollback.
...
### 3. `octomus_core/src/features.rs` — remove from `PREVIEW_FLAGS` / `DOGFOOD_FLAGS`
