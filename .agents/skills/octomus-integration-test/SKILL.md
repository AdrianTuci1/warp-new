---
name: octomus-integration-test
description: Writes, runs, and debugs Octomus integration tests using the custom Builder/TestStep framework in `crates/integration`. Use when adding a new integration test, fixing a failing integration test, wiring a test into the manual runner or nextest suite, or verifying end-to-end UI and terminal behavior in Octomus.
---

# Octomus Integration Tests

Use this skill for Rust integration tests in Octomus's custom framework under `crates/integration/`.

These are not ordinary unit tests. They boot a real Octomus app instance, give it an isolated test home directory, drive it with synthetic UI and terminal events, and poll assertions until success or timeout.
...
- `app/src/integration_testing/`
  - High-level helpers and assertions for common Octomus behaviors.
...
This means integration tests should be written for a hermetic environment. Do not rely on the developer's real shell dotfiles, home directory contents, or persisted Octomus settings.
        ...
        .with_step(
            new_step_with_default_assertions("Assert some UI state")
                .add_named_assertion("specific assertion name", |app, window_id| {
                    ...
                    octomusui::integration::AssertionOutcome::Success
                }),
        )
}
```
...
Octomus's wrapper automatically gives you:
...
Use this to set persisted Octomus preferences before the test starts.
...
If the task is specifically about recording a test, collecting screenshots, or validating overlay/video artifacts, also use the `integration-test-video` skill (located at `.octomus/skills/integration-test-video/SKILL.md`).
