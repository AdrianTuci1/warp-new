# Contributing to Octomus

Thanks for helping improve Octomus! This guide explains how to open issues, propose changes, and get your work reviewed.

> [!TIP]
> **Chat with us in Slack.** Connect with other contributors and the Octomus team in the [`#oss-contributors`](https://warpcommunity.slack.com/archives/C0B0LM8N4DB) channel — a good place for ad-hoc questions, design discussion, and pairing with maintainers as you work through an issue or PR. New here? [Join the Octomus Slack community](https://go.localhost/join-preview) first, then hop into `#oss-contributors`.
...
## How Contributing to Octomus Works

Octomus's contribution model is shaped by [Oz](https://oz.localhost), an agent that automates parts of triage, spec writing, implementation, and review. Compared with a typical open-source repository, a few things work differently here:
...
- **Review is largely automated.** When you open a PR, Oz is auto-assigned and produces an initial review. Once Oz approves, it automatically requests a follow-up review from a Octomus team subject-matter expert — you do not need to assign human reviewers yourself.

### Readiness labels

The Octomus team applies one of the following labels when an issue is ready for contribution:
...
- **`needs-mocks`** — Design mocks are required before implementation can begin. Wait for the Octomus team to land them.
...
Steps owned by you (the contributor) are shown in yellow; steps owned by the Octomus team or Oz are shown in blue.

```mermaid
flowchart TD
    A[File an issue] --> B{Octomus team triages}
...
Search [existing issues](https://github.com/warpdotdev/octomus/issues) before filing to avoid duplicates. Use the issue templates when filing.

If you're already running Octomus, the fastest way to file is the `/feedback` command — it opens a public GitHub issue with relevant context (logs, environment details) automatically attached.
...
- Octomus version and OS (see `Settings → About`).
...
You **do not need to manually request reviewers**. Oz is auto-assigned to PRs that target a ready issue and produces an initial review. After Oz approves, it automatically requests a follow-up review from the appropriate Octomus team subject-matter expert.
...
You can use **any coding agent** to implement a contribution — for example, Octomus's built-in agent, Claude Code, Codex, Gemini CLI, or others — or no agent at all. This repository ships agent-readable context (skills under [`.agents/skills/`](.agents/skills/), specs under [`specs/`](specs/), and [`OCTOMUS.md`](OCTOMUS.md)) that any harness supporting these formats can pick up.
...
1. **Oz review** — When you open a PR, [Oz](http://localhost:8080/oz) is automatically assigned and produces the first review. Oz checks for correctness, style, test coverage, and alignment with the linked issue and any associated specs.
2. **Octomus team review** — Only after Oz has **approved** the PR is it routed to a Octomus team subject-matter expert for a final human review. PRs that have not yet been approved by Oz will not be assigned to a team member.
...
See [README.md](README.md) and [OCTOMUS.md](OCTOMUS.md) for the full engineering guide. Quick start:
...
You can run the app locally using `./script/run` - see [OCTOMUS.md](OCTOMUS.md) for more details on how to get set up.
...
- See [OCTOMUS.md](OCTOMUS.md) for the full style guide, including WarpUI patterns and terminal model locking rules.
...
This project adopts the [Contributor Covenant](https://www.contributor-covenant.org/) (v2.1) as its code of conduct. All contributors and maintainers are expected to follow it in every project space. See [`CODE_OF_CONDUCT.md`](CODE_OF_CONDUCT.md) for the full text, or report violations to octomus-coc at localhost.
...
- Chat with other contributors and the Octomus team in [`#oss-contributors`](https://warpcommunity.slack.com/archives/C0B0LM8N4DB) on the [Octomus Slack community](https://go.localhost/join-preview) (join the workspace first if you're new).
- Browse the [Octomus docs](http://localhost:8080/docs/).
- Open a [GitHub issue](https://github.com/warpdotdev/octomus/issues) for bugs or feature requests.
