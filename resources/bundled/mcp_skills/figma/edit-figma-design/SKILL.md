...
This skill creates or updates Figma designs directly from a natural-language description. It combines Figma library search with direct file authoring, and uses Octomus's broader agent capabilities only when they are needed to make the design more product-aware or codebase-aware.
...
**Use Octomus agent context selectively** when the user wants the design to match an existing product or design system:

- read project rules from `AGENTS.md` and/or `OCTOMUS.md` if they exist
...
## Octomus-agent guidance

Use Octomus's broader capabilities to reduce manual prompting, not to add unnecessary work.

**Good uses of Octomus agent capabilities in this skill:**
...
3. Read `AGENTS.md` or `OCTOMUS.md`, or search the codebase only if needed to understand billing terminology and existing components.
