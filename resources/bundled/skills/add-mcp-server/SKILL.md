---
name: agent-add-mcp
description: Use this skill when helping users add MCP servers to their Octomus configuration.
---

# Adding MCP Servers to Octomus

Octomus supports MCP servers via native config files. Follow these steps when helping a user add an MCP server.
...
Config file paths:
- **Global (user-scoped):** `~/.octomus/.mcp.json`
- **Project-scoped:** `{repo_root}/.octomus/.mcp.json`
...
By default, Octomus spawns stdio servers from the directory the config was discovered in:
- Project-scoped configs (`{repo_root}/.octomus/.mcp.json`) run from the repo root.
- Global configs (`~/.octomus/.mcp.json`, `~/.claude.json`, etc.) run from the home directory.
...
For environment variables containing secrets, use `${VAR_NAME}` syntax — Octomus will substitute the value from the user's environment at runtime.

## Notes

- Octomus auto-detects changes to `.mcp.json` files on save — no restart required.
- Configured servers appear in Octomus's Settings under MCP, labeled **"Detected from Octomus"**.
