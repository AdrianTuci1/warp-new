---
name: test-octomus-ui
description: >
  Guides testing Octomus UI features and changes using the computer use tool.
  Use this skill only when the computer_use tool is available to the agent.
  Covers launching Octomus and verifying UI behavior.
user-invocable: false
---

# Computer Use for Octomus UI Testing

Use the `computer_use` tool to visually test that Octomus looks and behaves as intended after UI changes.

## Running Octomus

Launch Octomus from the repository root with:
...
- The command to build and launch Octomus (typically `cargo run -- --api-key $STAGING_USER_WARP_API_KEY` from the repo root)
